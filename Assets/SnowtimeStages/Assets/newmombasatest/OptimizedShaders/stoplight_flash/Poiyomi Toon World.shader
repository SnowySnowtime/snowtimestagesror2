Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/021afe1cbb746574ebde34f75f02377f"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionTex = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,4,0,0)), float4(0,0,0,0));
				}
				else
				{
					emissionTex = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,4,0,0).xy) + _Time.x * 5.0);
				}
				...
				if (1.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(-20.0, 20.0, 4.0, 0.0);
				}
				if(0.0)
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, float4(1,4,0,0));
				if (0.0)
				{
