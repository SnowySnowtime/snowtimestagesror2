Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/0bee1a1514a189f40a14e8afe7c6d7f3"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(3,3,0,0)), float4(0.25,-0.5,0,0), 0.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 0.1 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 1.05);
			}
