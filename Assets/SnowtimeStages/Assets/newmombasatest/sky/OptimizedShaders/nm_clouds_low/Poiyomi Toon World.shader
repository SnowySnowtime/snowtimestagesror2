Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/88614042fca36ac47a394e3e37ca7cce"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyDetailColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 0.25 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 1.0);
			}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, float4(5,5,0,0));
				if (0.0)
				{
					mainUV = sharpSample(float4(0.00390625,0.00390625,256,256), mainUV);
				}
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex, _MainTex, mainUV, float4(0.25,0.25,0,0), 1.0);
