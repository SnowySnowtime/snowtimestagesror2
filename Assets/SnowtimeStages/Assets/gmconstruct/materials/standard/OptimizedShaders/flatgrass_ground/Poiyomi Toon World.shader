Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/7d1003d6d5e960b42b7d3747ec7f16c6"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyGlobalMaskVertexColors(in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (1 > 0)
				{
					applyToGlobalMask(poiMods, 1 - 1, 2, vcol.r);
				}
				if (0.0 > 0)
				{
					applyToGlobalMask(poiMods, 0.0 - 1, 2.0, vcol.g);
				}
				if (0.0 > 0)
				{
					applyToGlobalMask(poiMods, 0.0 - 1, 2.0, vcol.b);
				}
				if (0.0 > 0)
				{
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(2,2,0,0)), float4(0,0,0,0), 1.0).rgb * poiThemeColor(poiMods, float4(0,0,0,1), 0.0);
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(0,0,0,1), 0.0);
				#endif
			...
			{
				#if defined(PROP_DETAILMASK) || !defined(OPTIMIZER_ENABLED)
				poiMods.detailMask = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailMask, _MainTex, poiUV(poiMesh.uv[5.0], float4(0.0015,0.0015,0,0)), float4(0,0,0,0), 1.0).rg;
				#else
			...
			void calculateRGBNormals(inout PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (0.0 == 0)
				{
					rgbMask = POI2D_SAMPLER_PAN(_RGBMask, _trilinear_repeat, poiUV(poiMesh.uv[5.0], float4(0.0075,0.0075,0,0)), float4(0,0,0,0));
				}
				#endif
				if (0.0 == 1)
				{
