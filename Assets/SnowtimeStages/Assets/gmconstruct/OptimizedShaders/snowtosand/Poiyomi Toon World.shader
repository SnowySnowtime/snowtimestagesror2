Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/52097a13570c93345935c7f6d7caa798"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyGlobalMaskVertexColors(in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (13 > 0)
				{
					applyToGlobalMask(poiMods, 13 - 1, 1, vcol.r);
				}
				if (15 > 0)
				{
					applyToGlobalMask(poiMods, 15 - 1, 1, vcol.g);
				}
				if (13 > 0)
				{
					applyToGlobalMask(poiMods, 13 - 1, 1, vcol.b);
				}
				if (0.0 > 0)
				{
					applyToGlobalMask(poiMods, 0.0 - 1, 2.0, vcol.a);
				}
			}
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(8,8,0,0)), float4(0,0,0,0), 1.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 0.75 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 1.25);
			}
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				if (16.0 > 0)
				{
					poiMods.detailMask.r = maskBlend(poiMods.detailMask.r, poiMods.globalMask[16.0 - 1], 2.0);
				}
				if (0.0 > 0)
				{
				...
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0), 0.0), 1.0 * poiMods.detailMask.g);
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0.5,0.5), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[0.0], 16.0, 2.0, poiMods), red, 0.0, 0.0, 1.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 9.0, 2.0, poiMods), green, 0.0, 0.0, 0.0, 0.0);
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalGlobalMask = 15.0;
				Decal0.m_DecalGlobalMaskBlendType = 0.0;
				Decal0.m_DecalApplyGlobalMaskIndex = 16.0;
				Decal0.m_DecalApplyGlobalMaskBlendType = 0.0;
				Decal0.m_DecalTexture_ST = float4(1.2,1.2,0.5,0.5);
				...
				Decal0.m_DecalTiled = 1.0;
				Decal0.m_DecalBlendType = 20.0;
				...
				Decal1.m_DecalMaskChannel = 3.0;
				Decal1.m_DecalGlobalMask = 16.0;
				Decal1.m_DecalGlobalMaskBlendType = 0.0;
				Decal1.m_DecalApplyGlobalMaskIndex = 0.0;
				Decal1.m_DecalApplyGlobalMaskBlendType = 0.0;
				Decal1.m_DecalTexture_ST = float4(6,6,0.5,0.5);
				Decal1.m_DecalTexturePan = float4(0,0,0,0);
				Decal1.m_DecalTextureUV = 0.0;
				Decal1.m_DecalColor = float4(0.4019779,0.4019779,0.4019779,1);
				Decal1.m_DecalColorThemeIndex = 0.0;
				Decal1.m_DecalTiled = 1.0;
				Decal1.m_DecalBlendType = 8.0;
				...
				Decal2.m_DecalFaceMask = 0.0;
				Decal2.m_DecalMaskChannel = 2.0;
				Decal2.m_DecalGlobalMask = 13.0;
				Decal2.m_DecalGlobalMaskBlendType = 0.0;
				Decal2.m_DecalApplyGlobalMaskIndex = 0.0;
				Decal2.m_DecalApplyGlobalMaskBlendType = 0.0;
				Decal2.m_DecalTexture_ST = float4(6,6,0,0);
				Decal2.m_DecalTexturePan = float4(0,0,0,0);
				Decal2.m_DecalTextureUV = 0.0;
				Decal2.m_DecalColor = float4(0.4019779,0.4019779,0.4019779,1);
				Decal2.m_DecalColorThemeIndex = 0.0;
				Decal2.m_DecalTiled = 1.0;
				Decal2.m_DecalBlendType = 2.0;
				Decal2.m_DecalRotation = 0.0;
				Decal2.m_DecalScale = float4(1,1,1,0);
				Decal2.m_DecalSideOffset = float4(0,0,0,0);
				Decal2.m_DecalPosition = float4(0.5,0.5,0,0);
				Decal2.m_DecalRotationSpeed = 0.0;
				Decal2.m_DecalEmissionStrength = 0.0;
				Decal2.m_DecalBlendAlpha = 0.0;
				Decal2.m_DecalOverrideAlpha = 0.0;
				Decal2.m_DecalHueShiftColorSpace = 0.0;
				Decal2.m_DecalHueShiftSelectOrShift = 1.0;
				Decal2.m_DecalHueShiftEnabled = 0.0;
				Decal2.m_DecalHueShift = 0.0;
				Decal2.m_DecalHueShiftSpeed = 0.0;
				Decal2.m_DecalDepth = 0.0;
				Decal2.m_DecalHueAngleStrength = 0.0;
				Decal2.m_DecalChannelSeparationEnable = 0.0;
				Decal2.m_DecalChannelSeparation = 0.0;
				Decal2.m_DecalChannelSeparationPremultiply = 0.0;
				Decal2.m_DecalChannelSeparationHue = 0.0;
				Decal2.m_DecalChannelSeparationVertical = 0.0;
				Decal2.m_DecalChannelSeparationAngleStrength = 0.0;
				Decal2.m_DecalOverrideAlphaMode = 0.0;
				Decal2.m_DecalMirroredUVMode = 0.0;
				Decal2.m_DecalSymmetryMode = 0.0;
				Decal2.Init(decalMask);
				#if defined(POI_AUDIOLINK)
				Decal2.m_AudioLinkDecalScaleBand = 0.0;
				Decal2.m_AudioLinkDecalScale = float4(0,0,0,0);
				Decal2.m_AudioLinkDecalRotationBand = 0.0;
				Decal2.m_AudioLinkDecalRotation = float4(0,0,0,0);
				Decal2.m_AudioLinkDecalAlphaBand = 0.0;
				Decal2.m_AudioLinkDecalAlpha = float4(0,0,0,0);
				Decal2.m_AudioLinkDecalEmissionBand = 0.0;
				Decal2.m_AudioLinkDecalEmission = float4(0,0,0,0);
				Decal2.m_DecalRotationCTALBand = 0.0;
				Decal2.m_DecalRotationCTALSpeed = 0.0;
				Decal2.m_DecalRotationCTALType = 0.0;
				Decal2.m_AudioLinkDecalColorChord = 0.0;
				Decal2.m_AudioLinkDecalSideBand = 0.0;
				...
				Decal2.m_AudioLinkDecalChannelSeparationBand = 0.0;
				...
				{
					udonVideoAspectRatio = Decal2.GetVideoAspectRatio(_Udon_VideoTex_TexelSize.zw, 0.0, 1.0);
					if (0.0)
					{
						if (udonVideoTexAvailable)
						{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, float4(8,8,0.5,0.5));
				if (0.0)
				{
					mainUV = sharpSample(float4(0.00390625,0.00390625,256,256), mainUV);
				}
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex, _MainTex, mainUV, float4(0,0,0,0), 1.0);
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.2622507,0.2622507,0.2622507,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.2622507,0.2622507,0.2622507,1).a;
				#else
