Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/f3d5da17d269eb8408766a4c72081ffa"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[4.0], float4(64,64,0,0)), float4(512,512,0,0), 1.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 1.0 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 2.96);
			}
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (1.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(1.0, 2.0, 5.0, 2.0);
				}
				if(0.0)
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (0.0)
				{
					mainUV = sharpSample(float4(0.0004882813,0.0078125,2048,128), mainUV);
				}
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex, _MainTex, mainUV, float4(0.05,0,0,0), 0.0);
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.004556415,0.004556415,0.004556415,1).rgb, 0.0);
