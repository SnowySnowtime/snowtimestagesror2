Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/50e559baa5bc863478656cbdf569af67"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float4 green = float4(poiThemeColor(poiMods, float4(0.250646,0.250646,0.250646,1).rgb, 0.0), float4(0.250646,0.250646,0.250646,1).a);
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(16,16,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				green *= POI2D_SAMPLER_PAN_STOCHASTIC(_GreenTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(10,10,0,0), float4(1,1,0,0))), float4(0,-5,0,0).xy, 1.0);
				#endif
				#if defined(PROP_BLUETEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[3.0], 0.0, 2.0, poiMods), red, 0.0, 0.0, -0.075, 1.0);
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float emissionStrength0__1 = 10.0;
