Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/ee1aba821e9df7a46a1c37eda7080088"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.1157568,0.05882353,1,1).rgb, 0.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.1157568,0.05882353,1,1).rgb, 0.0);
				#endif
				...
				if (1.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(-6400.0, 32.0, 1.0, 4.0);
				}
				if(0.0)
				{
