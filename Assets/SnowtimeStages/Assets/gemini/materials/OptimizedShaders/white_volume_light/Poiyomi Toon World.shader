Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/9cf824eddcd71af4597b16fece854814"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(0.5,0.5,0,0)), float4(1,1,0,0), 1.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.8303634,0.7122642,1,1).rgb, 0.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.8303634,0.7122642,1,1).rgb, 0.0);
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, float4(1,1,0,0.01));
				if (0.0)
				{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.07344791,0.0246054,0.1436938,1).rgb, 0.0);
