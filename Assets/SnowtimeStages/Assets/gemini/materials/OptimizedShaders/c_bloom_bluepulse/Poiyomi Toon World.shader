Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/18616ca80bb2f5b4c9aa61f3cb2bf8e4"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionTex = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * 1.0);
				}
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.05882353,0.3463916,1,1).rgb, 0.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.05882353,0.3463916,1,1).rgb, 0.0);
				#endif
				...
				if (0.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(0.0, 1.0, 2.0, 0.0);
				}
				if(0.0)
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (0.0 == 3)
				{
					poiLight.directColor = max(poiLight.directColor, 0.2);
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor * min(10000, (0.2 * rcp(calculateluminance(poiLight.directColor)))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor * min(10000, (0.2 * rcp(calculateluminance(poiLight.indirectColor)))));
				}
				...
				if (1.0)
				{
					poiLight.directColor = min(poiLight.directColor, 0.2);
					poiLight.indirectColor = min(poiLight.indirectColor, 0.2);
				}
				if (0.0)
				{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0,0.1916477,1,1).rgb, 0.0);
