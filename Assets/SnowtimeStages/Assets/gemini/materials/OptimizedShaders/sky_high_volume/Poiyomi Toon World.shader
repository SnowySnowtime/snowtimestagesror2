Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/92c4be5d6b84a684fa9c8c4d6c0d2a14"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,1,0,0)), float4(2,1,0,0), 0.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 1.2 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 1.25);
			}
