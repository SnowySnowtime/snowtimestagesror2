Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/5a7ea165bbd4e194c880ee60347559f6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + -0.1);
				if (0.0 > 0)
				{
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float4 red = float4(poiThemeColor(poiMods, float4(0.2906642,0.2906642,0.2906642,1).rgb, 0.0), float4(0.2906642,0.2906642,0.2906642,1).a);
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(16,16,0,0), float4(1,1,0,0))), float4(-1,-1,0,0).xy, 1.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.196991,0.196991,0.196991,1).rgb, 0.0);
