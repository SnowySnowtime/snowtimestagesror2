Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/385214ea7cbb5104f8572176b46cbfb8"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_CUBEMAP) || !defined(OPTIMIZER_ENABLED)
				if (any(float4(0,-90,0,0).xyz) || any(float4(0,0,0,0).xyz))
				{
					CubeMapUV = RotateAroundYInDegrees(CubeMapUV.yxz, float4(0,-90,0,0).x + (float4(0,0,0,0).x * _Time.y)).yxz;
					CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xyz, float4(0,-90,0,0).y + (float4(0,0,0,0).y * _Time.y)).xyz;
					CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xzy, float4(0,-90,0,0).z + (float4(0,0,0,0).z * _Time.y)).xzy;
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (0.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						poiLight.vColor[index] = 1.0 ? MaxLuminance(unity_LightColor[index].rgb * poiLight.vAttenuation[index], 0.0) : unity_LightColor[index].rgb * poiLight.vAttenuation[index];
				...
				poiLight.attenuationStrength = 0.0;
				poiLight.directColor = 1.0 ? MaxLuminance(_LightColor0.rgb * poiLight.attenuation, 0.0) : _LightColor0.rgb * poiLight.attenuation;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				poiLight.indirectColor = 0;
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, 0.5);
				poiLight.indirectColor = 1.0 ? MaxLuminance(poiLight.indirectColor, 0.0) : poiLight.indirectColor;
				#endif
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), 1.0);
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), 1.0);
				poiLight.halfDir = normalize(poiLight.direction + poiCam.viewDir);
