Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/4400f9b980d4b1e4fa40da1015908fd6"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float4 red = float4(poiThemeColor(poiMods, float4(1,1,1,0.5019608).rgb, 0.0), float4(1,1,1,0.5019608).a);
				float4 green = float4(poiThemeColor(poiMods, float4(1,1,1,0.5019608).rgb, 0.0), float4(1,1,1,0.5019608).a);
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0))), float4(2,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				green *= POI2D_SAMPLER_PAN_STOCHASTIC(_GreenTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0))), float4(-1.5,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_BLUETEXTURE) || !defined(OPTIMIZER_ENABLED)
				blue *= POI2D_SAMPLER_PAN_STOCHASTIC(_BlueTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0.25), float4(1,1,0,0))), float4(-5,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_ALPHATEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[0.0], 0.0, 2.0, poiMods), red, 2.0, 0.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 0.0, 2.0, poiMods), green, 2.0, 0.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[2.0], 0.0, 2.0, poiMods), blue, 0.0, 0.0, 0.0, 1.0);
