Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/4d129a3dbef0cca47a707c887a77e24d"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_GLOBALMASKTEXTURE0) || !defined(OPTIMIZER_ENABLED)
				float4 gmcol0 = POI2D_SAMPLER_PAN(_GlobalMaskTexture0, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,4,0,0)), float4(2,2,0,0));
				if (0)
				{
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,4,0,0)), float4(0,2,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,4,0,0)), float4(4,4,0,0)) * 2 - 1;
				#else
				float4 flowVector1 = -1;
				#endif
				#if defined(PROP_DISTORTIONMASK) || !defined(OPTIMIZER_ENABLED)
				half distortionMask = POI2D_SAMPLER_PAN(_DistortionMask, _MainTex, poiMesh.uv[0.0], float4(0,0,0,0))[0.0];
				#else
				half distortionMask = 1;
				#endif
				half distortionStrength = 0.2;
				half distortionStrength1 = 0.01;
				#ifdef POI_AUDIOLINK
				
				if (AudioLinkIsAvailable() && 0.0 && 1.0)
				{
					distortionStrength += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, AudioLinkData(uint2(0, uint(0.0))).r);
					distortionStrength1 += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, AudioLinkData(uint2(0, uint(0.0))).r);
				}
				#endif
				flowVector *= distortionStrength;
				flowVector1 *= distortionStrength1;
				return poiMesh.uv[0.0] + ((flowVector.xy + flowVector1.xy) / 2) * distortionMask;
			}
			#endif
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal1.m_DecalMaskChannel = 0.0;
				Decal1.m_DecalGlobalMask = 0.0;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionTex = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[7.0], float4(4,4,0,0)), float4(1,1,0,0));
				}
				else
				{
					emissionTex = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(4,4,0,0).xy) + _Time.x * 5.0);
				}
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.03184688,1,0,1).rgb, 0.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.03184688,1,0,1).rgb, 0.0);
				#endif
				...
				if (0.0)
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[7.0], float4(1,1,0,0)) + (dot(pos, float4(0,-10,0,0).xyz) * 20.0) + _Time.x * 10.0).r;
						#endif
					}
					else
					{
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, poiUV(poiMesh.uv[7.0], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0))[0.0];
				#else
				...
				if (1.0 > 0)
				{
					emissionMask0 = maskBlend(emissionMask0, poiMods.globalMask[1.0 - 1], 2.0);
				}
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float emissionStrength0__1 = 20.0;
				...
				if (!0.0)
				{
					emissionTex__1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[7.0], float4(4,4,0,0)), float4(1,1,0,0));
				}
				else
				{
					emissionTex__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(4,4,0,0).xy) + _Time.x * 5.0);
				}
				emissionColor0__1 = emissionTex__1.rgb * lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(0.03184688,1,0,1).rgb, 0.0);
				emissionAlpha__1 = emissionTex__1.a;
				#else
				emissionColor0__1 = lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(0.03184688,1,0,1).rgb, 0.0);
				#endif
				...
				if (0.0)
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE1) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0__1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, poiUV(poiMesh.uv[7.0], _EmissionScrollingCurve_ST__1) + (dot(pos__1, float4(0,-10,0,0).xyz) * 20.0) + _Time.x * 10.0).r;
						#endif
					}
					else
					{
				...
				#if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask1, _MainTex, poiUV(poiMesh.uv[7.0], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0))[0.0];
				#else
				...
				if (1.0 > 0)
				{
					emissionMask0__1 = maskBlend(emissionMask0__1, poiMods.globalMask[1.0 - 1], 2.0);
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[7.0].xy, float4(3,3,0,0));
				if (0.0)
				{
					mainUV = sharpSample(float4(0.001953125,0.001953125,512,512), mainUV);
				}
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex, _MainTex, mainUV, float4(-1,1,0,0), 1.0);
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.2424458,0.957614,0.2181832,1).rgb, 0.0);
				...
				#endif
				if (4.0 == POI_MODE_OPAQUE)
				{
				}
				clip(poiFragData.alpha - 0.0);
				if (4.0 == POI_MODE_CUTOUT && !0.0)
				{
					poiFragData.alpha = 1;
				}
