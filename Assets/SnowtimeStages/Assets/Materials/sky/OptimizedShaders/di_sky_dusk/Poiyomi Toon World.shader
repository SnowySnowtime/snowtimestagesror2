Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/976909289a31dfd4fab62d0b5e57e5ff"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyGlobalMaskTextures(in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_GLOBALMASKTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 gmcol1 = POI2D_SAMPLER_PAN(_GlobalMaskTexture1, _MainTex, poiUV(poiMesh.uv[0.0], float4(64,64,0,0)), float4(1,-1,0,0));
				if (0)
				{
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalApplyGlobalMaskIndex = 2.0;
				Decal0.m_DecalApplyGlobalMaskBlendType = 2.0;
				Decal0.m_DecalTexture_ST = float4(1,1,0,0);
				Decal0.m_DecalTexturePan = float4(0.1,0.1,0,0);
				Decal0.m_DecalTextureUV = 0.0;
				Decal0.m_DecalColor = float4(0,0,0,1);
				...
				Decal2.m_DecalMaskChannel = 0.0;
				Decal2.m_DecalGlobalMask = 0.0;
				Decal2.m_DecalGlobalMaskBlendType = 2.0;
				Decal2.m_DecalApplyGlobalMaskIndex = 2.0;
				Decal2.m_DecalApplyGlobalMaskBlendType = 2.0;
				Decal2.m_DecalTexture_ST = float4(8,8,0,0);
				Decal2.m_DecalTexturePan = float4(0.2,0.2,0,0);
				Decal2.m_DecalTextureUV = 0.0;
				Decal2.m_DecalColor = float4(0,0,0,1);
				Decal2.m_DecalColorThemeIndex = 0.0;
				Decal2.m_DecalTiled = 0.0;
				Decal2.m_DecalBlendType = 0.0;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionTex = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(10,10,0,0)), float4(0.1,0.1,0,0));
				}
				else
				{
					emissionTex = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(10,10,0,0).xy) + _Time.x * 5.0);
				}
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, poiUV(poiMesh.uv[0.0], float4(11,11,0,0)) + _Time.x * float4(1,1,0,0))[0.0];
				#else
				...
				if (2.0 > 0)
				{
					emissionMask0 = maskBlend(emissionMask0, poiMods.globalMask[2.0 - 1], 2.0);
				}
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float emissionStrength0__1 = 2.0;
				...
				if (!0.0)
				{
					emissionTex__1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[0.0], float4(24,24,0,0)), float4(0.1,0.1,0,0));
				}
				else
				{
					emissionTex__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(24,24,0,0).xy) + _Time.x * 5.0);
				}
				...
				#if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask1, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0.25)) + _Time.x * float4(0,0,0,0))[0.0];
				#else
