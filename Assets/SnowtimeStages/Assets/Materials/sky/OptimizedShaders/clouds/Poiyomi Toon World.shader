Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/b3ab204e225b0cd408e05bc05455416c"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[0.0], float4(32,32,0,0)), float4(1,1,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex, poiUV(poiMesh.uv[0.0], float4(5,5,0,0)), float4(-0.75,0,0,0)) * 2 - 1;
				#else
				...
				#endif
				half distortionStrength = 0.02;
				half distortionStrength1 = 0.03;
				#ifdef POI_AUDIOLINK
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[7.0].xy, float4(1,1,0,0));
				if (0.0)
				{
