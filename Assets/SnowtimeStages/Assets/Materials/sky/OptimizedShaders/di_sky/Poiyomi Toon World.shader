Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/71f6d40eeb4c14e439aa43f68c228dea"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionTex__1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[0.0], float4(16,16,0,0)), float4(0.1,0.1,0,0));
				}
				else
				{
					emissionTex__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(16,16,0,0).xy) + _Time.x * 5.0);
				}
				...
				#if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask1, _MainTex, poiUV(poiMesh.uv[0.0], float4(11,11,0,0)) + _Time.x * float4(1,1,0,0))[0.0];
				#else
				...
				if (2.0 > 0)
				{
					emissionMask0__1 = maskBlend(emissionMask0__1, poiMods.globalMask[2.0 - 1], 2.0);
				}
