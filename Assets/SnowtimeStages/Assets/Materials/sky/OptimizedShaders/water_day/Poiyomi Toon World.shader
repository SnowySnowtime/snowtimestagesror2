Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/ce47f416c5bf401498d04de27f65461b"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(64,128,0,0)), float4(1,0,0,0), 1.0), 0.15 * poiMods.detailMask.g);
			...
			void calculateRGBNormals(inout PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (0.15 > 0 && 1.0)
				{
					float3 normalToBlendWith = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalR, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(512,512,0,0), float4(1,1,0,0) )), float4(1,1,0,0), 1.0), 1.0 == 0 ? 0.15 : 0.15 * maskFinal.r);
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float4 red = float4(poiThemeColor(poiMods, float4(0,0,0,1).rgb, 0.0), float4(0,0,0,1).a);
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 1.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(1.0)
				{
					...
					CubeMapUV = lerp(poiMesh.normals[0], poiMesh.normals[1], 1.0);
					break;
				...
				float4 cubeMap = texCUBElod(_CubeMap, float4(CubeMapUV, (1 - 1.0) * (1 - 1.0) * 8));
				cubeMap.rgb *= poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
				float4 cubeMap = float4(0.21763764082, 0.21763764082, 0.21763764082, .5) * float4(poiThemeColor(poiMods, float4(1,1,1,1), 0.0), 1);
				#endif
				cubeMap.rgb *= 0.15;
				#if defined(PROP_CUBEMAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (0.0)
				{
					cubeMap.rgb = hueShift(cubeMap.rgb, 0.0 + _Time.x * 0.0, 0.0, 1.0);
					cubeMap = PoiColorBCS(cubeMap, 0.0, 1.0, 1.0);
				}
				CubeMapMask = min(CubeMapMask, lerp(1, poiLight.rampedLightMap, 0.0));
				float cubeMapAlpha = CubeMapMask * cubeMap.a * 1.0;
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, cubeMap.rgb, cubeMapAlpha * (2.0 == 0));
				poiFragData.baseColor.rgb *= lerp(1, cubeMap.rgb, cubeMapAlpha * (2.0 == 1));
				poiFragData.baseColor.rgb += cubeMap.rgb * cubeMapAlpha * (2.0 == 2);
				poiFragData.emission += cubeMap.rgb * 1.0 * CubeMapMask * cubeMap.a;
			}
			#endif
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (1.0 == 1)
				{
					...
					specular2 = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1), 0.0) * poiLight.occlusion * attenuation * 0.5;
					if (poiFragData.toggleVertexLights)
					{
						#if defined(VERTEXLIGHT_ON)
						for (int index = 0; index < 4; index++)
						{
							...
							float pbrVDotNH = lerp(poiLight.vertexVDotNH[index], poiLight.vDotNH[index], 1.0);
							GetSpecFresTerm(pbrNDotL, pbrNDotV, pbrNDotH, poiLight.lDotH, specularTerm, fresnelTerm, specCol, brdfRoughness);
							vSpecular2 += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1), 0.0) * poiLight.occlusion * 0.5;
						}
						#endif
					}
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0].xy, float4(768,768,0,0)), float4(1,1,0,0), 1.0), 0.15);
				#else
				...
				#endif
				if (5.0 == POI_MODE_OPAQUE)
				{
				}
				clip(poiFragData.alpha - 0.0);
				if (5.0 == POI_MODE_CUTOUT && !0.0)
				{
					poiFragData.alpha = 1;
				}
