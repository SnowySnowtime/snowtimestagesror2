Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/6d389267c8a1f6949b49d967b7d78906"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(128,256,0,0)), float4(1,0,0,0), 1.0), 0.15 * poiMods.detailMask.g);
			...
			void calculateRGBNormals(inout PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (0.15 > 0 && 1.0)
				{
					float3 normalToBlendWith = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalR, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1024,1024,0,0), float4(1,1,0,0) )), float4(1.5,1,0,0), 1.0), 1.0 == 0 ? 0.15 : 0.15 * maskFinal.r);
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 cubeMap = texCUBElod(_CubeMap, float4(CubeMapUV, (1 - 0.8) * (1 - 0.8) * 8));
			...
			void MetallicAndSpecularFragDataInit(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MOCHIEMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PBRMaps = POI2D_SAMPLER_PAN_STOCHASTIC(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(4,4,0,0), 0.0);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0].xy, float4(4096,4096,0,0)), float4(1,1,0,0), 1.0), 0.15);
				#else
				...
				if (0.0)
				{
					poiLight.directColor = min(poiLight.directColor, 0.0);
					poiLight.indirectColor = min(poiLight.indirectColor, 0.0);
				}
				if (0.0)
				{
