Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/50cbfee67861b6149a6988440b767e09"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionTex__1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,0.24,0,0)), float4(0,0,0,0));
				}
				else
				{
					emissionTex__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,0.24,0,0).xy) + _Time.x * 5.0);
				}
				emissionColor0__1 = emissionTex__1.rgb * lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(0.5176471,1.45098,2,1).rgb, 0.0);
				emissionAlpha__1 = emissionTex__1.a;
				#else
				emissionColor0__1 = lerp(1, poiFragData.baseColor, mixBaseColor__1).rgb * poiThemeColor(poiMods, float4(0.5176471,1.45098,2,1).rgb, 0.0);
				#endif
			...
			void applyPathing(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (1.0)
				{
					pathSourceOrMask = POI2D_SAMPLER_PAN(_PathingMap, _point_repeat, poiUV(poiMesh.uv[0.0], float4(1,0.24,0,0)), float4(0,0,0,0));
				}
				else
				{
					pathSourceOrMask = POI2D_SAMPLER_PAN(_PathingMap, _linear_repeat, poiUV(poiMesh.uv[0.0], float4(1,0.24,0,0)), float4(0,0,0,0));
				}
				#endif
				float4 path;
				if (0.0 == 0)
				...
				#if defined(PROP_PATHINGCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				float4 pathColorMap = POI2D_SAMPLER_PAN(_PathingColorMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,0.24,0,0)), float4(0,0,0,0));
				#else
				float4 pathColorMap = float4(1, 1, 1, 1);
				#endif
				float4 PathColor[4] = {
					float4(0.09960464,0.2589721,5.079837,1), float4(1,1,1,1), float4(1,1,1,1), float4(3.984185,11.07603,15.21959,1)
				};
				float4 pathTypes = float4(4.0, 4.0, 4.0, 2.0);
				...
				if (poiMods.audioLinkAvailable)
				{
					if (0.0)
					{
						...
						timeAudioValues.x = poiMods.audioLink[(int)0.0];
						timeAudioValues.y = poiMods.audioLink[(int)0.0];
						timeAudioValues.z = poiMods.audioLink[(int)0.0];
						timeAudioValues.w = poiMods.audioLink[(int)0.0];
						alTimeOffsets = lerp(alTimeOffsetMins, alTimeOffsetMaxs, timeAudioValues);
					}
					if (0.0)
					{
						float4 chronoBands = float4(0.0, 0.0, 0.0, 0.0);
						float4 chronoTypes = float4(0.0, 0.0, 0.0, 0.0);
						float4 chronoSpeeds = float4(0.0, 0.0, 0.0, 0.0);
						alTimeOffsets += AudioLinkGetChronoTime(chronoTypes, chronoBands) * chronoSpeeds;
					}
					if (0.0)
					{
						float4 alWidthOffsetMins = float4(float4(0,0,0,1).x, float4(0,0,0,1).x, float4(0,0,0,1).x, float4(0,0,0,1).x);
						float4 alWidthOffsetMaxs = float4(float4(0,0,0,1).y, float4(0,0,0,1).y, float4(0,0,0,1).y, float4(0,0,0,1).y);
						float4 widthAudioValues;
						widthAudioValues.x = poiMods.audioLink[(int)0.0];
					...
					if (0.0)
					{
						float4 alEmissionMins = float4(float4(0,0,0,1).x, float4(0,0,0,1).x, float4(0,0,0,1).x, float4(0,0,0,1).x);
						float4 alEmissionMaxs = float4(float4(0,0,0,1).y, float4(0,0,0,1).y, float4(0,0,0,1).y, float4(0,0,0,1).y);
				...
				{
					#ifdef POI_AUDIOLINK
					float timeOffset = 0.0 || 0.0 ? alTimeOffsets[i] : 0;
					#else
					float timeOffset = 0;
					#endif
					float currentTime = frac((float4(-999,-999,-999,-999)[i] == -999.0f ? _Time.y * float4(0.1,0.1,0.1,0.5)[i] : float4(-999,-999,-999,-999)[i]) + float4(0,0,0,0)[i] + timeOffset);
					...
					#ifdef POI_AUDIOLINK
					half pathWidth = float4(0.03,0.03,0.03,0.1)[i] * 0.5 + (0.0 ? alWidthOffsets[i] : 0);
					#else
					half pathWidth = float4(0.03,0.03,0.03,0.1)[i] * 0.5;
					#endif
					half rcpPathWidth = rcp(pathWidth + 1e-6);
					half softness = max(float4(1,1,1,1)[i], 0);
					half fillAlpha;
					if (softness > 1e-5)
					{
					...
					half dashAlpha = 0;
					float totalDashLength = float4(0.03,0.03,0.03,0.1)[i] + float4(0.1,0.1,0.1,0.1)[i];
					if (totalDashLength > 1e-6)
					{
						float relativeDashLength = float4(0.03,0.03,0.03,0.1)[i] / totalDashLength;
					...
					if (poiMods.audioLinkAvailable)
					{
						if (0.0)
						{
							float4 historyToggles = float4(0.0, 0.0, 0.0, 0.0);
				...
				pathEmission += finalPathAlpha.r * poiThemeColor(poiMods, PathColor[0].rgb, 0.0) * (float4(0,0,0,0).r + pathAudioLinkEmissionAdd.r);
				pathEmission += finalPathAlpha.g * poiThemeColor(poiMods, PathColor[1].rgb, 0.0) * (float4(0,0,0,0).g + pathAudioLinkEmissionAdd.g);
				pathEmission += finalPathAlpha.b * poiThemeColor(poiMods, PathColor[2].rgb, 0.0) * (float4(0,0,0,0).b + pathAudioLinkEmissionAdd.b);
				pathEmission += finalPathAlpha.a * poiThemeColor(poiMods, PathColor[3].rgb, 0.0) * (float4(0,0,0,0).a + pathAudioLinkEmissionAdd.a);
