Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/03a533d3b11a0db4fb741a536f6fe4de"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				cubeMap.rgb *= poiThemeColor(poiMods, float4(1,0.463734,0.09714396,1), 0.0);
				#else
				float4 cubeMap = float4(0.21763764082, 0.21763764082, 0.21763764082, .5) * float4(poiThemeColor(poiMods, float4(1,0.463734,0.09714396,1), 0.0), 1);
				#endif
				cubeMap.rgb *= 1.0;
				#if defined(PROP_CUBEMAPMASK) || !defined(OPTIMIZER_ENABLED)
