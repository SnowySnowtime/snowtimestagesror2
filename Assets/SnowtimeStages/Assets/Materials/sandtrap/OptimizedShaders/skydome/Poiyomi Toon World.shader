Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/2ded21c193b02f34396ee62e0ca6dbbf"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + 1.0);
				if (0.0 > 0)
				{
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,4,0,0)), float4(-4,0,0,0), 1.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 2.0 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 0.03);
			}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[8.0].xy, float4(1,1,0,0));
				if (0.0)
				{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(1,0.9440049,0.866134,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(1,0.9440049,0.866134,1).a;
				#else
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (1.2) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(lerp(poiFragData.baseColor, poiFragData.baseColor * (0.4 + 1), hueShiftAlpha.g));
				#endif
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
