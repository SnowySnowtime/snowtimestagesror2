Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/9b1f70fd8e8d371459b3b58b65950bf7"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(100,100,0,0)), float4(0,0,0,0), 1.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 2.0 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 0.4);
			}
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float4 alpha = float4(poiThemeColor(poiMods, float4(0,0,0,1).rgb, 0.0), float4(0,0,0,1).a);
				#if defined(PROP_RGBMASK) || !defined(OPTIMIZER_ENABLED)
				if (0.0 == 0)
				{
					rgbMask = POI2D_SAMPLER_PAN(_RGBMask, _trilinear_repeat, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(5,5,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				green *= POI2D_SAMPLER_PAN_STOCHASTIC(_GreenTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(100,100,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_BLUETEXTURE) || !defined(OPTIMIZER_ENABLED)
				blue *= POI2D_SAMPLER_PAN_STOCHASTIC(_BlueTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(30,30,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_ALPHATEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 0.0, 2.0, poiMods), red, 0.0, 0.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 0.0, 2.0, poiMods), green, 0.0, 20.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[2.0], 0.0, 2.0, poiMods), blue, 0.0, 0.0, 0.75, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 0.0, 2.0, poiMods), alpha, 0.0, 0.0, 0.0, 0.0);
				if (1.0 || 0.0 || 0.0 || 1.0)
				{
					#if defined(PROP_RGBASMOOTHNESSMAPS) || !defined(OPTIMIZER_ENABLED)
					...
					if (0.0 && 1.0 && 1.0)
					{
						smoothnessMaps.r = POI2D_SAMPLER_PAN_STOCHASTIC(_RGBASmoothnessMaps, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 1.0).r;
					}
					if (0.0 && 0.0 && 1.0)
					{
					...
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.r, 0.0), rgbMask[1.0] * (1.0 && 1.0));
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.g, 0.0), rgbMask[1.0] * (0.0 && 1.0));
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.b, 0.0), rgbMask[2.0] * (0.0 && 1.0));
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.a, 0.0), rgbMask[1.0] * (1.0 && 0.0));
					#endif
					#if defined(PROP_RGBAMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
					...
					if (0.0 && 1.0 && 1.0)
					{
						metallicMaps.r = POI2D_SAMPLER_PAN_STOCHASTIC(_RGBAMetallicMaps, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 1.0).r;
					}
					if (0.0 && 0.0 && 1.0)
					{
					...
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.r, 1.0), rgbMask[1.0] * (1.0 && 1.0));
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.g, 1.0), rgbMask[1.0] * (0.0 && 1.0));
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.b, 1.0), rgbMask[2.0] * (0.0 && 1.0));
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.a, 0.0), rgbMask[1.0] * (1.0 && 0.0));
					#endif
				}
			}
			#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, float4(10,10,0,0));
				if (0.0)
				{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(1,0.9386859,0.6104956,1).rgb, 0.0);
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (-0.35) * hueShiftAlpha.b);
