Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/afe10647a79948b40ac33d22f839c528"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + -0.9);
				if (0.0 > 0)
				{
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[0.0], float4(7,2,0,0)), float4(-2,1,0,0), 1.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				lightMap.x *= lerp(1.0, shadowAttenuationNoStrength, 1.0);
				lightMap.y *= lerp(1.0, shadowAttenuationNoStrength, 1.0);
				lightMap.z *= lerp(1.0, shadowAttenuationNoStrength, 1.0);
				...
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 1.0 * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 1.0 * blurMap.r, 0.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 1.0 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.5, 0.3 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 1.0 * blurMap.r, 0.0);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalTexture_ST = float4(10.5,5,0,0);
				Decal0.m_DecalTexturePan = float4(-10,0,0,0);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (0.0)
				{
					mainUV = sharpSample(float4(0.0009765625,0.00390625,1024,256), mainUV);
				}
