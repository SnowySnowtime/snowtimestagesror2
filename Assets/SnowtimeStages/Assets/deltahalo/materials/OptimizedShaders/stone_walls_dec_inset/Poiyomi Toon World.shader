Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/0bdf60a36f5b9cd40b2d3265cbf7119b"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				green *= POI2D_SAMPLER_PAN_STOCHASTIC(_GreenTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(18,18,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_BLUETEXTURE) || !defined(OPTIMIZER_ENABLED)
				blue *= POI2D_SAMPLER_PAN_STOCHASTIC(_BlueTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(7,7,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_ALPHATEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 4.0, 2.0, poiMods), green, 0.0, 9.0, 1.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[2.0], 0.0, 2.0, poiMods), blue, 0.0, 9.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[3.0], 0.0, 2.0, poiMods), alpha, 0.0, 9.0, -0.2, 1.0);
				if (0.0 || 0.0 || 0.0 || 0.0)
				{
					#if defined(PROP_RGBASMOOTHNESSMAPS) || !defined(OPTIMIZER_ENABLED)
