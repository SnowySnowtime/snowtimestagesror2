Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/e21f590a6a3b3fe4d9a7c3ee815e2720"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applySubsurfaceScattering(in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiFragData poiFragData)
			{
				...
				SSS.a = 1 - SSS.a;
				#endif
				float3 vLTLight = poiLight.direction + poiMesh.normals[0] * 0.0;
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), 2.8) * 1.0;
				#ifdef UNITY_PASS_FORWARDBASE
				float3 fLT = (flTDot) * saturate(SSS.a + - 1 * -1.0);
				#else
				float3 fLT = poiLight.additiveShadow * (flTDot) * saturate(SSS.a + - 1 * -1.0);
				#endif
				#if defined(POINT) || defined(SPOT)
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(0.196991,0.196991,0.196991,1) * SSS.rgb * lerp(1, poiFragData.baseColor, 0.0);
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(0.196991,0.196991,0.196991,1) * SSS.rgb * poiLight.attenuation * lerp(1, poiFragData.baseColor, 0.0);
			}
			#endif
