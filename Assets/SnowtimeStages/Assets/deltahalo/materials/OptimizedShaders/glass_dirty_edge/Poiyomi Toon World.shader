Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/daece5641c4700d43ae69797f13e38f0"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_CUBEMAPMASK) || !defined(OPTIMIZER_ENABLED)
				float CubeMapMask = POI2D_SAMPLER_PAN(_CubeMapMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_CubeMapMaskUV*/)], float4(3,2,0,0)), float4(0,0,0,0))[(3.0 /*_CubeMapMaskChannel*/)];
				#else
				float CubeMapMask = 1;
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[(0.0 /*_MainTexUV*/)].xy, float4(3,2,0,0));
