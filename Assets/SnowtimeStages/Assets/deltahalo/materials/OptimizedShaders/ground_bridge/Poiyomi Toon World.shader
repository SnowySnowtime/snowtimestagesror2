Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/88f5e903825bb6c4590ed668439a0781"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(50,50,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				#if defined(PROP_ALPHATEXTURE) || !defined(OPTIMIZER_ENABLED)
				alpha *= POI2D_SAMPLER_PAN_STOCHASTIC(_AlphaTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(50,50,0,0), float4(1,1,0,0))), float4(0,0,0,0).xy, 0.0);
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[0.0], 0.0, 2.0, poiMods), red, 0.0, 0.0, 1.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 6.0, 2.0, poiMods), green, 0.0, 0.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[2.0], 3.0, 2.0, poiMods), blue, 0.0, 0.0, 0.0, 1.0);
