Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/e93eb03f4c39fd442a0d293597e89b08"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateRGBNormals(inout PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (0.0 > 0) maskFinal.g = customBlend(maskFinal.g, poiMods.globalMask[0.0 - 1], 7.0);
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,4,0,0), float4(1,1,0,0))), float4(0,4,0,0).xy, 0.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				green *= POI2D_SAMPLER_PAN_STOCHASTIC(_GreenTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,4,0,0), float4(1,1,0,0))), float4(0,6,0,0).xy, 0.0);
				#endif
				#if defined(PROP_BLUETEXTURE) || !defined(OPTIMIZER_ENABLED)
				blue *= POI2D_SAMPLER_PAN_STOCHASTIC(_BlueTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,4,0,0), float4(1,1,0,0))), float4(0,8,0,0).xy, 0.0);
				#endif
				#if defined(PROP_ALPHATEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[3.0], 0.0, 2.0, poiMods), red, 0.0, 0.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[3.0], 0.0, 2.0, poiMods), green, 0.0, 0.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[3.0], 0.0, 2.0, poiMods), blue, 0.0, 0.0, 0.0, 1.0);
				...
				if (0.0 || 0.0 || 0.0 || 0.0)
				{
					...
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.r, 0.0), rgbMask[3.0] * (0.0 && 1.0));
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.g, 0.0), rgbMask[3.0] * (0.0 && 1.0));
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.b, 0.0), rgbMask[3.0] * (0.0 && 1.0));
					...
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.r, 0.0), rgbMask[3.0] * (0.0 && 1.0));
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.g, 0.0), rgbMask[3.0] * (0.0 && 1.0));
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.b, 0.0), rgbMask[3.0] * (0.0 && 1.0));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.finalLighting = max(POI_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, indirectLight).xyz, 0.25);
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0].xy, float4(1,1,0,0)), float4(0,0,0,0), 0.0), 0.2);
				#else
				...
				if (0.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						poiLight.vColor[index] = 0.0 ? MaxLuminance(unity_LightColor[index].rgb * poiLight.vAttenuation[index], 0.25) : unity_LightColor[index].rgb * poiLight.vAttenuation[index];
				...
				if (0.0 == 3)
				{
					poiLight.directColor = max(poiLight.directColor, 0.25);
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor * min(10000, (0.25 * rcp(calculateluminance(poiLight.directColor)))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor * min(10000, (0.25 * rcp(calculateluminance(poiLight.indirectColor)))));
				}
				...
				if (1.0)
				{
					poiLight.directColor = min(poiLight.directColor, 0.25);
					poiLight.indirectColor = min(poiLight.indirectColor, 0.25);
				}
				if (0.0)
				{
				...
				poiLight.attenuationStrength = 0.0;
				poiLight.directColor = 0.0 ? MaxLuminance(_LightColor0.rgb * poiLight.attenuation, 0.25) : _LightColor0.rgb * poiLight.attenuation;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				poiLight.indirectColor = 0;
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, 0.0);
				poiLight.indirectColor = 0.0 ? MaxLuminance(poiLight.indirectColor, 0.25) : poiLight.indirectColor;
				#endif
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), 1.0);
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), 1.0);
				poiLight.halfDir = normalize(poiLight.direction + poiCam.viewDir);
