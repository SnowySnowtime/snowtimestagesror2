 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (1.0)
				{
					...
					float distanceFadeMultiplier = lerp(0.0, 0.5, smoothstep(15.0, 40.0, distance(position, poiCam.worldPos)));
					if (0.0 > 0)
					{
			...
			void applyDithering(inout PoiFragData poiFragData, in PoiCam poiCam)
			{
				if (1.0)
				{
					float dither = calcDither(poiCam.posScreenPixels) - 0.313;
					poiFragData.alpha = saturate(poiFragData.alpha - (dither * (1 - poiFragData.alpha) * 0.373));
				}
			}
