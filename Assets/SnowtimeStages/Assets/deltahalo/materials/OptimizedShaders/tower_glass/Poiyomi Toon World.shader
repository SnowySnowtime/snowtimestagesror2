Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/1fa2ac19737ec6541ac5f49a1ec93b35"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float cubeMapAlpha = CubeMapMask * cubeMap.a * 0.057;
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0].xy, float4(1,1,0,0)), float4(0,0,0,0), 1.0), 0.2);
				#else
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.0890862,0.0890862,0.0890862,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.0890862,0.0890862,0.0890862,1).a;
				#else
