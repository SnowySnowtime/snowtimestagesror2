Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/782e9bd0328e8de4784fa4ae7d103435"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyDetailColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * 1.0 * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * 1.0);
			}
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(88,16,0,0)), float4(4,-2,0,0), 1.0), 0.03 * poiMods.detailMask.g);
			...
			void calculateRGBNormals(inout PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				float4 maskFinal = 1;
				maskFinal.r = rgbMask[3.0];
				maskFinal.g = rgbMask[3.0];
				maskFinal.b = rgbMask[3.0];
				...
				if (0.02 > 0 && 1.0)
				{
					float3 normalToBlendWith = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalR, _trilinear_repeat, poiUV(poiMesh.uv[5.0], PoiUVMerge(float4(0.1,0.1,0,0), float4(1,1,0,0) )), float4(1,-0.2,0,0), 1.0), 1.0 == 0 ? 0.02 : 0.02 * maskFinal.r);
				...
				if (0.015 > 0 && 1.0)
				{
					float3 normalToBlendWith = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalG, _trilinear_repeat, poiUV(poiMesh.uv[5.0], PoiUVMerge(float4(0.04,0.04,0,0), float4(1,1,0,0) )), float4(1,0.5,0,0), 1.0), 1.0 == 0 ? 0.015 : 0.015 * maskFinal.g);
				...
				if (0.01 > 0 && 1.0)
				{
					float3 normalToBlendWith = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalB, _trilinear_repeat, poiUV(poiMesh.uv[5.0], PoiUVMerge(float4(0.15,0.15,0,0), float4(1,1,0,0))), float4(-1,-1,0,0), 1.0), 1.0 == 0 ? 0.01 : 0.01 * maskFinal.b);
				...
				if (0.03 > 0 && 1.0)
				{
					float3 normalToBlendWith = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalA, _trilinear_repeat, poiUV(poiMesh.uv[5.0], PoiUVMerge(float4(0.05,0.015,0,0), float4(1,1,0,0))), float4(7,-2,0,0), 1.0), 1.0 == 0 ? 0.03 : 0.03 * maskFinal.a);
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				red *= POI2D_SAMPLER_PAN_STOCHASTIC(_RedTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(50,50,0,0), float4(1,1,0,0))), float4(-0.2,0.3,0,0).xy, 0.0);
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
				green *= POI2D_SAMPLER_PAN_STOCHASTIC(_GreenTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(60,60,0,0), float4(1,1,0,0))), float4(0.1,-0.3,0,0).xy, 0.0);
				#endif
				#if defined(PROP_BLUETEXTURE) || !defined(OPTIMIZER_ENABLED)
				blue *= POI2D_SAMPLER_PAN_STOCHASTIC(_BlueTexture, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(30,30,0,0), float4(1,1,0,0))), float4(0,-0.15,0,0).xy, 0.0);
				#endif
				#if defined(PROP_ALPHATEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[0.0], 0.0, 2.0, poiMods), red, 0.0, 0.0, -0.1, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[0.0], 0.0, 2.0, poiMods), green, 0.0, 0.0, -0.1, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[0.0], 0.0, 2.0, poiMods), blue, 0.0, 0.0, -0.1, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[3.0], 0.0, 2.0, poiMods), alpha, 0.0, 2.0, 0.0, 1.0);
				if (0.0 || 0.0 || 0.0 || 0.0)
				{
					#if defined(PROP_RGBASMOOTHNESSMAPS) || !defined(OPTIMIZER_ENABLED)
					...
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.g, 0.0), rgbMask[0.0] * (0.0 && 1.0));
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.b, 0.0), rgbMask[0.0] * (0.0 && 1.0));
					poiFragData.smoothness = lerp(poiFragData.smoothness, poiInvertToggle(smoothnessMaps.a, 0.0), rgbMask[3.0] * (0.0 && 1.0));
					#endif
					#if defined(PROP_RGBAMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
					...
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.g, 0.0), rgbMask[0.0] * (0.0 && 1.0));
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.b, 0.0), rgbMask[0.0] * (0.0 && 1.0));
					poiFragData.metallic = lerp(poiFragData.metallic, poiInvertToggle(metallicMaps.a, 0.0), rgbMask[3.0] * (0.0 && 1.0));
					#endif
				}
			}
			#endif
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				float shadowStrength = 0.0 * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
			...
			void MetallicAndSpecularFragDataInit(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (0.0)
				{
					...
					assignValueToVectorFromIndex(PBRMaps, 4.0, PBRSplitMask[4.0]);
				}
				if (2.0 < 4)
				{
					metallic *= PBRMaps[2.0];
				}
				if (2.0 < 4)
				{
					smoothness *= PBRMaps[2.0];
					smoothness2 *= PBRMaps[2.0];
				}
				if (2.0 < 4)
				{
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (0.0)
				{
					...
					percepRough = GSAA_Filament(normals, percepRough, 0.067, 1.0);
					if (0.0 == 1 && 1.0 > 0)
					{
						percepRough2 = GSAA_Filament(normals, percepRough2, 0.067, 1.0);
					}
				}
				...
				float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, roughness, 1.0, 1.0, _MochieReflCube, _MochieReflCube_HDR, pbrReflectionDir);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
					float alphaMask = POI2D_SAMPLER_PAN(_AlphaMask, _MainTex, poiUV(poiMesh.uv[5.0], float4(1,1,0,0)), float4(0,0,0,0).xy).r;
					#else
					float alphaMask = 1;
					#endif
