#ifndef FOLIAGE_UTILS
#define FOLIAGE_UTILS
...
float3 WaveGrass (const float4 vertex, float4 waveParams, const float waveAmount)
{
    ...
    float3 output = 0;
    output.xz -= waveMove.xz * waveParams.z;
    return output;
}
...
float3 TrembleGrass(float3 vertex, float3 normal, float waveAmount)
{
	...
	float3 displacement = (0.065 * sin(2.650 * (vertex.x + vertex.y + vertex.z + _Time.y)))
		* (1, 1, 0.35) * normal;
    return displacement * waveAmount;

	// Also, grass motion todo:
	//worldCentre.w = _Time.y;
	//motionX = (waveFac * 0.01 * 2 * sin( 1 * dot(worldCentre, 1.0))) + 1;
	//motionZ = (waveFac * 0.01 * 1 * sin( 2 * dot(worldCentre, 1.0))) + 0.5;
}
