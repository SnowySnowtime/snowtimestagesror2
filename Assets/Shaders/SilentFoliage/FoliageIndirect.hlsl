#ifndef FOLIAGE_INDIRECT
#define FOLIAGE_INDIRECT
...
inline UnityGI UnityGI_Base_local(UnityGIInput data, out half bakedAtten, inout half occlusion, 
	half3 normalWorld, float smoothness, half3 transmission, float exposureOcclusion, float maxSmoothness,
    float probeTransmission, SSSParams sssData)
{
    ...
    // VRC Light Volumes also have an additive component which can be added over lightmapping.
        ...
        o_gi.indirect.diffuse += Irradiance_SampleVRCLightVolumeAdditive(normalWorld, data.worldPos, filamentLight);

        // Merge lights, weighing each light's contribution by their intensity
        float derivedLum = luminance(filamentLight.colorIntensity.rgb);
        ...
        filamentLight.l = normalize(filamentLight.l * derivedWeight + volumeLight.l * volumeWeight);
        filamentLight.colorIntensity = filamentLight.colorIntensity * derivedWeight + volumeLight.colorIntensity * volumeWeight;
        filamentLight.attenuation = filamentLight.attenuation * derivedWeight + volumeLight.attenuation * volumeWeight;
        filamentLight.NoL = filamentLight.NoL * derivedWeight + volumeLight.NoL * volumeWeight;
    #endif
