#ifndef SPLAT_SSR_INCLUDED
#define SPLAT_SSR_INCLUDED
...
float3 GetBlurredGP(const float2 texelSize, const float2 uvs, const float dim){
	...
	for (int i = 0; i < dimFloored; i++){
		for (int j = 0; j < dimFloored; j++){
			float4 refl = MOCHIE_SAMPLE_TEX2D_SCREENSPACE(_SplatGrab, float2(uvs.x + pixSize.x*(i-center), uvs.y + pixSize.y*(j-center)));
			refTotal += refl.rgb;
		}
	}
...
float4 GetSSR(const float3 wPos, const float3 viewDir, float3 rayDir, const half3 faceNormal, float smoothness, float3 albedo, float metallic, float4 screenPos){
	...
	if (IsInMirror() || FdotR < 0 || roughness > 0.65){
		...
		noiseUvs.xy = (noiseUvs.xy * _SplatGrab_TexelSize.zw) / (_NoiseTexSSR_TexelSize.zw * noiseUvs.w);	
		...
		if (reflectionAlpha > 0){
			...
			// if (blurFac > 1){
				reflection.rgb = GetBlurredGP(_SplatGrab_TexelSize.zw, uvs.xy, blurFac);
			// }
			// else {
			// 	reflection.rgb = MOCHIE_SAMPLE_TEX2D_SCREENSPACE(_SplatGrab, uvs.xy).rgb;
			// }
