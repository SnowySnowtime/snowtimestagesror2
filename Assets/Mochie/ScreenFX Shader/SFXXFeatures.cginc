...
float2 GetFogRadius(v2f i, float3 pos, float range, float falloff){
    GetDepth(i, wPos, wNorm, depth);
    float distXZ = distance(wPos.xz, pos.xz);
	float distY = distance(wPos.y, pos.y);
    float radiusXZ = smoothstep(range - falloff, range, distXZ);
	float heightY = smoothstep(range - falloff, range, distY);
	float2 radius = float2(radiusXZ, heightY);
	// if (_HeightFalloff == 1)
	// 	radius *= smoothstep(_FogHeightMax, _FogHeightMin, wPos.y);
    return radius;
}

float2 GetFogRadiusError(v2f i, float3 pos, float range, float falloff){
	GetDepth(i, wPos, wNorm, depth);
	float3 cam2Pos = wPos - pos;
	float dist = length(cam2Pos);
	float angle = acos(dot(cam2Pos / dist, float3(0,1,0))); // remember angle is in radians!
	float angleFade = smoothstep(0.59, .6, angle);
	float distanceFade = smoothstep(range - falloff, range, dist);
	return angleFade;
}
...
void ApplyFog(v2f i, inout float4 col, audioLinkData ald){
	...
	// float2 fogRadius = GetFogRadius(i, fogSpace, _FogRadius, _FogFade);
	// float radius = fogRadius.x;
	// float height = fogRadius.y;
...
float3 GetAura(v2f i, float3 bg, audioLinkData ald){
	_AuraStr *= 25;
	#if AUDIOLINK_ENABLED
		float audioLinkMultiplier = GetAudioLinkBand(ald, _AudioLinkOutlineStrength, _AudioLinkOutlineBand, _AudioLinkOutlineMin, _AudioLinkOutlineMax);
		float audioLinkColorMultiplier = GetAudioLinkBand(ald, _AudioLinkOutlineColStrength, _AudioLinkOutlineColBand, _AudioLinkOutlineColMin, _AudioLinkOutlineColMax);
	...
	if (_OutlineCubeToggle == 1){
		_CubeRotate = lerp(_CubeRotate, _CubeRotate *_Time.y, _AutoRotate);
		float3 vDir = Rotate3D(i.viewDir, _CubeRotate);
		_OutlineCol.rgb = texCUBE(_OutlineCube, vDir).rgb * _OutlineCol.rgb;
	}
	if (_OutlineHueShiftAuto == 1){
		_OutlineHueShift = frac(_Time.y * _OutlineHueShiftSpeed * 0.25);
	}
	if (_OutlineHueShift > 0 && _OutlineHueShift < 1){
		_OutlineCol.rgb = HSVShift(_OutlineCol.rgb, _OutlineHueShift, 0, 0);
	}
	#if AUDIOLINK_ENABLED
		interpolator *= audioLinkColorMultiplier;
	#endif
	return lerp(bg, (1-blurDepth)*_OutlineCol.rgb, interpolator);
}
...
float3 SoftOutline(v2f i, float3 bg, audioLinkData ald){
	...
	if (_OutlineHueShift > 0 && _OutlineHueShift < 1){
		_OutlineCol.rgb = HSVShift(_OutlineCol.rgb, _OutlineHueShift, 0, 0);
		_OutlineColAlt.rgb = HSVShift(_OutlineColAlt.rgb, _OutlineHueShift, 0, 0);
	}
	float3 col = 0;
	if (_OutlineAltEnable == 1){
		#if AUDIOLINK_ENABLED
			float audioLinkColorMultiplier = GetAudioLinkBand(ald, _AudioLinkOutlineColStrength, _AudioLinkOutlineColBand, _AudioLinkOutlineColMin, _AudioLinkOutlineColMax);
			float audioLinkMixMultiplier = GetAudioLinkBand(ald, _AudioLinkOutlineMixStrength, _AudioLinkOutlineMixBand, _AudioLinkOutlineMixMin, _AudioLinkOutlineMixMax);
			_OutlineThresh *= audioLinkColorMultiplier;
			_OutlineThreshAlt *= audioLinkMixMultiplier;
		#endif
		if (_OutlineCubeToggle == 1){
			_CubeRotateAlt = lerp(_CubeRotateAlt, _CubeRotateAlt *_Time.y, _AutoRotateAlt);
			float3 vDirAlt = Rotate3D(i.viewDir, _CubeRotateAlt);
			_OutlineColAlt.rgb = texCUBE(_OutlineCubeAlt, vDirAlt).rgb * _OutlineColAlt.rgb;
		}
		float maxThresh = 45;
		float percentA = _OutlineThresh / maxThresh;
		float percentB = _OutlineThreshAlt / maxThresh;

		float interpolatorA = saturate(sobel*_OutlineThresh) * _OutlineCol.a;
		float interpolatorB = saturate(sobel*_OutlineThreshAlt) * _OutlineColAlt.a;

		float3 a = lerp(bg, _OutlineCol.rgb*percentA, interpolatorA);
		float3 b = lerp(bg, _OutlineColAlt.rgb*percentB, interpolatorB);
		col = (a+b) * 0.5;
	}
	else {
		float interpolator = saturate(sobel*_OutlineThresh)*_OutlineCol.a;
		col = lerp(bg, _OutlineCol.rgb, interpolator);
	}
	return col;
}
...
void ApplyOutline(v2f i, inout float3 col, audioLinkData ald){
	float3 bg = 0;
	if (_OutlineCubeToggle == 1){
		_CubeRotateBG = lerp(_CubeRotateBG, _CubeRotateBG *_Time.y, _AutoRotateBG);
		float3 vDir = Rotate3D(i.viewDir, _CubeRotateBG);
		bg = lerp(col, texCUBE(_OutlineBackgroundCube, vDir).rgb * _BackgroundCol.rgb, _BackgroundCol.a);
	}
	else {
		bg = lerp(col, _BackgroundCol.rgb, _BackgroundCol.a);
	}
	...
	if (_OutlineType == 1){
		...
		col = lerp(col, GetAura(i, bg, ald), falloff);
	}
}
