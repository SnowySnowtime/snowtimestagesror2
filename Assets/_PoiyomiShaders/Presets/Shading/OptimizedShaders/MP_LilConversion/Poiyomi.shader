Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/bf8c5ab91bf6aaa49b949bf74998ddc2"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				else if (1.0 == 1) // Toon
				{
					...
					#endif
					if (0.6 == 0.4) 0.6 += 0.001;
					poiLight.rampedLightMap = smoothstep(0.6, 0.4, 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(0.4, 0.6, 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(0.4, 0.6, 1 - (.5 * poiLight.nDotL + .5)));
					#endif
					return;
				}
				#endif
				...
				lightMap.x *= lerp(1.0, poiLight.attenuation, 0.0);
				lightMap.y *= lerp(1.0, poiLight.attenuation, 0.0);
				lightMap.z *= lerp(1.0, poiLight.attenuation, 0.0);
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 0.1 * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 0.1 * blurMap.r, 0.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 0.1 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.5, 0.3 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 0.1 * blurMap.r, 0.0);
				}
				lns = saturate(lns);
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, 0.0);
				...
				if (1.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (1.0 == 1) // Toon
						{
							vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * 0.5 * poiLight.vAttenuation[index], smoothstep(0.4, 0.6, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow;
						}
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						poiLight.vColor[index] = 1.0 ? min(1.0, unity_LightColor[index].rgb) : unity_LightColor[index].rgb;
				...
				if (3.0 == 0) // Poi Custom Light Color
				{
				...
				if (3.0 == 1) // More standard approach to light color
				{
				...
				if (3.0 == 3)
				{
					poiLight.directColor = max(poiLight.directColor, 0.05);
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor * min(10000, (0.05 * rcp(calculateluminance(poiLight.directColor)))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor * min(10000, (0.05 * rcp(calculateluminance(poiLight.indirectColor)))));
				}
				...
				poiLight.directColor = 1.0 ? min(1.0, _LightColor0.rgb) : _LightColor0.rgb;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
