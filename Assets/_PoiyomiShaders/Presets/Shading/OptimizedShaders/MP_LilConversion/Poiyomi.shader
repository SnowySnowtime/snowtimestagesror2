 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				else if ((1.0 /*_LightingAdditiveType*/) == 1) // Toon
				{
					...
					#endif
					if ((0.6 /*_LightingAdditiveGradientEnd*/) == (0.4 /*_LightingAdditiveGradientStart*/)) (0.6 /*_LightingAdditiveGradientEnd*/) += 0.001;
					poiLight.rampedLightMap = smoothstep((0.6 /*_LightingAdditiveGradientEnd*/), (0.4 /*_LightingAdditiveGradientStart*/), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.4 /*_LightingAdditiveGradientStart*/), (0.6 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.4 /*_LightingAdditiveGradientStart*/), (0.6 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
					return;
				}
				#endif
				...
				if ((1.0 /*_LightingVertexLightingEnabled*/))
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						{
							vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * (0.5 /*_LightingAdditivePassthrough*/) * poiLight.vAttenuation[index], smoothstep((0.4 /*_LightingAdditiveGradientStart*/), (0.6 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow;
						}
					}
