Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/0dfb68f3451a286408b0be70c899a6c2"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				firstShadeMap.rgb *= float4(0.7254258,0.7254258,0.7254258,1).rgb; //* lighColor
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if ((0.0 /*_LightingAdditiveType*/) == 0) // Realistic
				{
				...
				else if ((0.0 /*_LightingAdditiveType*/) == 1) // Toon
				{
					...
					#endif
					if ((0.5 /*_LightingAdditiveGradientEnd*/) == (0.5 /*_LightingAdditiveGradientStart*/)) (0.5 /*_LightingAdditiveGradientEnd*/) += 0.001;
					poiLight.rampedLightMap = smoothstep((0.5 /*_LightingAdditiveGradientEnd*/), (0.5 /*_LightingAdditiveGradientStart*/), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.5 /*_LightingAdditiveGradientStart*/), (0.5 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.5 /*_LightingAdditiveGradientStart*/), (0.5 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
					return;
				}
				#endif
				...
				if ((1.0 /*_LightingVertexLightingEnabled*/))
				{
					...
					{
						if ((0.0 /*_LightingAdditiveType*/) == 0 || (0.0 /*_LightingAdditiveType*/) == 3)
						{
						...
						{
							vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * (0.5 /*_LightingAdditivePassthrough*/) * poiLight.vAttenuation[index], smoothstep((0.5 /*_LightingAdditiveGradientStart*/), (0.5 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow;
						}
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.4865309,0.4865309,0.4865309,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.4865309,0.4865309,0.4865309,1).a;
