Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/0dfb68f3451a286408b0be70c899a6c2"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				#endif
				firstShadeMap = lerp(firstShadeMap, float4(poiFragData.baseColor, 1), 1.0);
				#if defined(PROP_2ND_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				mainShadowMask *= poiLight.shadowMask * 1.0;
				firstSecondShadowMask *= poiLight.shadowMask * 1.0;
				if (0.0 == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				else if (0.0 == 1) // Toon
				{
					...
					#endif
					if (0.5 == 0.5) 0.5 += 0.001;
					poiLight.rampedLightMap = smoothstep(0.5, 0.5, 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(0.5, 0.5, 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(0.5, 0.5, 1 - (.5 * poiLight.nDotL + .5)));
					#endif
					return;
				}
				#endif
				...
				if (1.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (0.0 == 1) // Toon
						{
							vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * 0.5 * poiLight.vAttenuation[index], smoothstep(0.5, 0.5, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow;
						}
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.4865309,0.4865309,0.4865309,1).rgb, 0.0);
