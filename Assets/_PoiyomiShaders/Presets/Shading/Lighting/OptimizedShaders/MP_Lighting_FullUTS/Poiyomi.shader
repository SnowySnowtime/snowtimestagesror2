Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/b3ea4f0c69d93b14ca1f17342c9502fa"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if ((1.0 /*_LightingVertexLightingEnabled*/))
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						{
							vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * (1.0 /*_LightingAdditivePassthrough*/) * poiLight.vAttenuation[index], smoothstep((1.0 /*_LightingAdditiveGradientStart*/), (1.0 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow;
						}
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.4453062,0.4453062,0.4453062,1).rgb, (0.0 /*_ColorThemeIndex*/));
