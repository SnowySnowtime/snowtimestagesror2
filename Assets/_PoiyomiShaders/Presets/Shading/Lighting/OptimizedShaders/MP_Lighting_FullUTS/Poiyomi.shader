Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/b3ea4f0c69d93b14ca1f17342c9502fa"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				else if (1.0 == 1) // Toon
				{
					...
					#endif
					if (1.0 == 1.0) 1.0 += 0.001;
					poiLight.rampedLightMap = smoothstep(1.0, 1.0, 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(1.0, 1.0, 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(1.0, 1.0, 1 - (.5 * poiLight.nDotL + .5)));
					#endif
					return;
				}
				#endif
				...
				if (1.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (1.0 == 1) // Toon
						{
							vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * 1.0 * poiLight.vAttenuation[index], smoothstep(1.0, 1.0, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow;
						}
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, 1.0);
				#endif
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), 0.0);
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), 0.0);
				poiLight.halfDir = normalize(poiLight.direction + poiCam.viewDir);
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.4453062,0.4453062,0.4453062,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.4453062,0.4453062,0.4453062,1).a;
