Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon Grab Pass/8d183e97b99f26040a156cbc60955feb"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (1.0)
				{
					float holoRim = saturate(1 - smoothstep(min(0.067, 0.763), 0.763, (poiCam.vDotN)));
					holoRim = abs(lerp(1, holoRim, 0.875));
					holoRim = 0.0 ? 1 - holoRim : holoRim;
					if (0.0 > 0)
					{
			...
			float4 grabpassBlur(float2 uv, float4 grabPos, float smoothness)
			{
				...
				float2 radius = (0.036 + smoothness) * .1;
				radius.x *= aspect; // Adjust for aspect ratio
				float quality = floor(5.0);
				float directions = floor(5.0);
			...
			inline float4 Refraction(float indexOfRefraction, float chromaticAberration, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, inout PoiLight poiLight, in PoiFragData poiFragData)
			{
				...
				else
				{
					...
					refractionColor = grabpassBlur(refractedGrab, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * 0.5);
					#else
			...
			void applyGrabEffects(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (1.0 == 1)
				{
					grabpassColor = Refraction(1.3, 0.0, poiMesh, poiCam, poiMods, poiLight, poiFragData).rgb;
				}
				else
				{
					#ifdef CHROMATIC_ABERRATION_LOW
					grabpassColor = grabpassBlur(poiCam.screenUV, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * 0.5);
					#else
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0].xy, float4(2,5,0,0)), float4(0,0,0,0), 0.0), 0.36);
				#else
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.08839609,0.471447,0.7986599,0.4313726).rgb, 0.0);
