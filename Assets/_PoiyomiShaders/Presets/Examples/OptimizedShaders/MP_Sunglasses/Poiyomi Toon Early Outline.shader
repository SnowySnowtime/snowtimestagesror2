Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon Outline Early/e882f4d57a954904b8a144bdcea017ea"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				if (0.0)
				{
					distanceOffset *= lerp(1.0, clamp((distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, o.localPos).xyz)), 0.0f, 9999.0), 0.5);
				}
				float lineWidth = 0.0;
				#ifdef POI_AUDIOLINK
				if (1.0)
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				float4 outlineColor = float4(0.9799461,0.3631918,1,1);
				#ifdef POI_AUDIOLINK
