Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/4ccb17d0fa7ada74190025227a9dc06d"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (poiMods.audioLinkAvailable)
				{
					if (1.0)
					{
						currentFrame += AudioLinkGetChronoTime(1.0, 1.0) * 20.0;
					}
				...
				if (poiMods.audioLinkAvailable)
				{
					flipbookEmissionStrength += max(lerp(float4(0,6,0,0).x, float4(0,6,0,0).y, poiMods.audioLink[2.0]), 0);
				}
				#endif
