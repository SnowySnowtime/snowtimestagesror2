Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/67ca07f5ecca1804ea63f70fdfa360f5"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float flipBookMask = POI2D_SAMPLER_PAN(_FlipbookMask, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0))[0.0];
				#else
				float flipBookMask = 1;
				#endif
				if (0.0 > 0)
				{
					flipBookMask = maskBlend(flipBookMask, poiMods.globalMask[0.0-1], 2.0);
				}
				float4 flipbookScaleOffset = float4(1,1,0,0);
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
					flipbookScaleOffset.xy += lerp(float4(0,0,0,0).xy, float4(0,0,0,0).zw, poiMods.audioLink[0.0]);
				}
				#endif
				flipbookScaleOffset.xy = 1 - flipbookScaleOffset.xy;
				float2 uv = frac(poiMesh.uv[0.0] +_Time.x * float4(0,0,0,1).xy);
				...
				if (0.0)
				{
					totalFrames -= (totalFrames - min(max(0.0, 0.0), totalFrames));
					totalFrames -= max(0, 0.0);
				}
				if (!0.0)
				{
					if (14.0 != 0)
					{
						currentFrame = ((_Time.y / (1 / 14.0)) + 0.0) % totalFrames;
						if (0.0)
						{
							currentFrame += 0.0;
						}
				...
				{
					currentFrame = fmod(0.0, totalFrames);
				}
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
					if (1.0)
					{
						currentFrame += AudioLinkGetChronoTime(1.0, 1.0) * 8.0;
					}
					currentFrame += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[0.0]);
					float totalFramesAL = totalFrames;
					if (0.0)
					{
						totalFramesAL += max(0, 0.0);
					}
				...
				if (0.0)
				{
					flipBookPixel.rgb = poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0);
				}
				else
				{
					flipBookPixel.rgb *= poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0);
				...
				if (0.0)
				{
					flipBookPixel.rgb = hueShift(flipBookPixel.rgb, 0.0 + _Time.x * 0.0, 0.0, 1.0);
				}
				half flipbookAlpha = 1;
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
					flipbookAlpha += saturate(lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[0.0]));
				}
				#endif
				#if !defined(POI_PASS_OUTLINE) && !defined(UNITY_PASS_SHADOWCASTER)
				poiFragData.baseColor = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, flipBookPixel.rgb, 8.0), flipBookPixel.a * float4(1,1,1,1).a * 1.0 * flipBookMask * flipbookAlpha);
				float flipbookEmissionStrength = 4.0;
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
					flipbookEmissionStrength += max(lerp(float4(0,0.5,0,0).x, float4(0,0.5,0,0).y, poiMods.audioLink[2.0]), 0);
				}
				#endif
