Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/1b554d513234cef468b0778acdec93c5"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (poiMods.audioLinkAvailable)
				{
					if (1.0)
					{
						currentFrame += AudioLinkGetChronoTime(1.0, 1.0) * 24.0;
					}
				...
				if (poiMods.audioLinkAvailable)
				{
					flipbookEmissionStrength += max(lerp(float4(0,20,0,0).x, float4(0,20,0,0).y, poiMods.audioLink[2.0]), 0);
				}
				#endif
