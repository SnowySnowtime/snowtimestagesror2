Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/bab679e29a5dc0842b989b06e3c58b18"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint(2.0); glitterLayer++)
				{
					...
					if (1.0)
					{
						size = saturate(size + lerp(float4(0,0.4,0,0).x, float4(0,0.4,0,0).y, poiMods.audioLink[0.0]));
					}
					#endif
					half glitterAlpha = 1;
					switch(0.0)
					...
					if (1.0)
					{
						glitterSpeedOffset += AudioLinkGetChronoTime(0.0, 0.0) * 0.2;
					}
					#endif
					switch(0.0)
					{
					...
					if (1.0)
					{
						glitterMask = clamp(glitterMask + lerp(float4(0,0.9,0,0).x, float4(0,0.9,0,0).y, poiMods.audioLink[1.0]), 0, glitterMask);
					}
					#endif
					if (0.0)
					{
					...
					if (1.0)
					{
						GlitterbrightnessOffset = max(GlitterbrightnessOffset +lerp(float4(0,100,0,0).x, float4(0,100,0,0).y, poiMods.audioLink[2.0]), 0);
					}
					#endif
