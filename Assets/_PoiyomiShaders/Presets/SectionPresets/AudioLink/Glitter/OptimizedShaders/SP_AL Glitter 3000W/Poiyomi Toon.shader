Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/6da4ec4deb4ed3d4088216c8ce0aba67"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint(2.0); glitterLayer++)
				{
					...
					if (1.0)
					{
						size = saturate(size + lerp(float4(0,0.5,0,0).x, float4(0,0.5,0,0).y, poiMods.audioLink[0.0]));
					}
					#endif
					half glitterAlpha = 1;
					switch(0.0)
					...
					if (1.0)
					{
						glitterSpeedOffset += AudioLinkGetChronoTime(0.0, 0.0) * 0.3;
					}
					#endif
					switch(0.0)
					{
					...
					if (1.0)
					{
						glitterMask = clamp(glitterMask + lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[1.0]), 0, glitterMask);
					}
					#endif
					if (0.0)
					{
					...
					if (1.0)
					{
						GlitterbrightnessOffset = max(GlitterbrightnessOffset +lerp(float4(0,800,0,0).x, float4(0,800,0,0).y, poiMods.audioLink[2.0]), 0);
					}
					#endif
