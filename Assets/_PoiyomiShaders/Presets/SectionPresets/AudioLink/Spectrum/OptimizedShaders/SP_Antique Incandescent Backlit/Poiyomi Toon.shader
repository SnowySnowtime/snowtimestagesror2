Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/e521e7368b5839d4d927e507e15593eb"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				#if defined(PROP_ALDECALCOLORMASK) || !defined(OPTIMIZER_ENABLED)
				colorAndMask = POI2D_SAMPLER_PAN(_ALDecalColorMask, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				#endif
				if (0.0 > 0)
				{
				...
				float maskVolume = 10.0 != 0.0 ? floor(maskY * 10.0) / 10.0 : maskY;
				float maskBand = 10.0 != 0.0 ? floor(maskX * 10.0) / 10.0 : maskX;
				audioLinkMask.r = maskVolume;
				audioLinkMask.g = maskBand;
				audioLinkMask.b = maskVolume < 0.0 || maskVolume > 1.0 ? 0.0 : audioLinkMask.b;
				audioLinkMask.b = maskBand < 0.0 || maskBand > 1.0 ? 0.0 : audioLinkMask.b;
				if (1.0)
				{
					float volumeth = 0.586;
					if (10.0 != 0.0) audioLinkMask.b = frac(maskY * 10.0) > volumeth ? 0.0 : audioLinkMask.b;
					float bandwidth = 0.0 == 1 ? 0.879 / aluv.y : 0.879;
					float bandth = 1.0 - bandwidth;
					if (10.0 != 0.0) audioLinkMask.b = frac(maskX * 10.0 + bandth * 0.5) < bandth ? 0.0 : audioLinkMask.b;
				}
				...
				float audioVal = audioTexture.b * 1.0 * lerp(5.0, 1.0, audioLinkMask.g);
				float audioLinkValue = 1.0 < 1.0 ? abs(audioVal - audioLinkMask.r) < 1.0 : audioVal > audioLinkMask.r * 2.0;
				...
				float3 lowColor = float4(1,0.6920712,0.4479884,1).rgb * poiThemeColor(poiMods, float4(1,0.6920712,0.4479884,1).rgb, 0.0);
				float3 midColor = float4(1,0.693872,0.4507858,1).rgb * poiThemeColor(poiMods, float4(1,0.693872,0.4507858,1).rgb, 0.0);
				float3 highColor = float4(1,0.693872,0.4507858,1).rgb * poiThemeColor(poiMods, float4(1,0.693872,0.4507858,1).rgb, 0.0);
				...
				float3 emissionColor = lerp(lowColor * 2.0, midColor * 2.0, saturate(volumeColorSrc * 2));
				emissionColor = lerp(emissionColor, highColor * 2.0, saturate(volumeColorSrc * 2 - 1));
				#if defined(POI_PASS_BASE) || defined(POI_PASS_ADD)
				poiFragData.emission += emissionColor * audioLinkValue;
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor, customBlend(poiFragData.baseColor, volumeColor * colorAndMask.rgb, 8.0), saturate(1.0 * audioLinkValue));
				#endif
				poiFragData.alpha = lerp(poiFragData.alpha, poiFragData.alpha * audioLinkValue, 0.0);
			}
			#endif
			#endif
