Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/79e9030983879a946865745af96856b7"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float maskVolume = 12.0 != 0.0 ? floor(maskY * 12.0) / 12.0 : maskY;
				float maskBand = 12.0 != 0.0 ? floor(maskX * 12.0) / 12.0 : maskX;
				...
				{
					float volumeth = 0.95;
					if (12.0 != 0.0) audioLinkMask.b = frac(maskY * 12.0) > volumeth ? 0.0 : audioLinkMask.b;
					float bandwidth = 0.0 == 1 ? 0.95 / aluv.y : 0.95;
					float bandth = 1.0 - bandwidth;
					if (12.0 != 0.0) audioLinkMask.b = frac(maskX * 12.0 + bandth * 0.5) < bandth ? 0.0 : audioLinkMask.b;
				}
				...
				float3 lowColor = float4(0.4019778,1,0,1).rgb * poiThemeColor(poiMods, float4(0.4019778,1,0,1).rgb, 0.0);
				float3 midColor = float4(0.4019779,1,0,1).rgb * poiThemeColor(poiMods, float4(0.4019779,1,0,1).rgb, 0.0);
				float3 highColor = float4(0.4019779,1,0,1).rgb * poiThemeColor(poiMods, float4(0.4019779,1,0,1).rgb, 0.0);
				...
				emissionColor = lerp(emissionColor, highColor * 3.0, saturate(volumeColorSrc * 2 - 1));
