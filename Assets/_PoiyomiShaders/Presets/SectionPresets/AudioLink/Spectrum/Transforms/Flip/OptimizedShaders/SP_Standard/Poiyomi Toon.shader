Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/4b2f66c1368172243a40266842df0569"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float maskVolume = 0.0 != 0.0 ? floor(maskY * 0.0) / 0.0 : maskY;
				float maskBand = 0.0 != 0.0 ? floor(maskX * 0.0) / 0.0 : maskX;
				...
				{
					float volumeth = 0.5;
					if (0.0 != 0.0) audioLinkMask.b = frac(maskY * 0.0) > volumeth ? 0.0 : audioLinkMask.b;
					float bandwidth = 0.0 == 1 ? 0.5 / aluv.y : 0.5;
					float bandth = 1.0 - bandwidth;
					if (0.0 != 0.0) audioLinkMask.b = frac(maskX * 0.0 + bandth * 0.5) < bandth ? 0.0 : audioLinkMask.b;
				}
				...
				float audioVal = audioTexture.b * 0.5 * lerp(5.0, 1.0, audioLinkMask.g);
				...
				float3 lowColor = float4(1,0.1587067,0.002428216,1).rgb * poiThemeColor(poiMods, float4(1,0.1587067,0.002428216,1).rgb, 0.0);
				float3 midColor = float4(1,0.7449472,0.5549318,1).rgb * poiThemeColor(poiMods, float4(1,0.7449472,0.5549318,1).rgb, 0.0);
				float3 highColor = float4(0.3005438,0.4286905,0.8148468,1).rgb * poiThemeColor(poiMods, float4(0.3005438,0.4286905,0.8148468,1).rgb, 0.0);
				...
				float3 emissionColor = lerp(lowColor * 0.1, midColor * 1.0, saturate(volumeColorSrc * 2));
				emissionColor = lerp(emissionColor, highColor * 1.0, saturate(volumeColorSrc * 2 - 1));
