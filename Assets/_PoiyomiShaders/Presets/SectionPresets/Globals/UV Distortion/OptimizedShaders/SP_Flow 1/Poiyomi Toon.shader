Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/71205da691058794f8d3917649dbb715"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,4,0,0)), float4(2,0,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex, poiUV(poiMesh.uv[0.0], float4(3,3,0,0)), float4(0,-2,0,0)) * 2 - 1;
				#else
