Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/45b9bd23025ab5c4e801ed2a1737d796"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[0.0], float4(6,6,0,0)), float4(2,0,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex, poiUV(poiMesh.uv[0.0], float4(7,7,0,0)), float4(1,0,0,0)) * 2 - 1;
				#else
