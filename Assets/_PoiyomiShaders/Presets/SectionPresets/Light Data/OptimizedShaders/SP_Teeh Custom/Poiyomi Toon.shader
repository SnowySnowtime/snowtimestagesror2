Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/9392d94f15870434facaa649d54973f0"
{
	Properties
	{
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (lightingMode == 1)
						{
							float2 ToonAddGradient = float2(0.0, 0.5);
							if (ToonAddGradient.x == ToonAddGradient.y) ToonAddGradient.y += 0.0001;
							vertexLighting = max(vertexLighting, lerp(poiLight.vColor[index], poiLight.vColor[index] * 0.05, smoothstep(ToonAddGradient.x, ToonAddGradient.y, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow);
						}
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						poiLight.vColor[index] = 1.0 ? MaxLuminance(unity_LightColor[index].rgb * poiLight.vAttenuation[index], 2.5) : unity_LightColor[index].rgb * poiLight.vAttenuation[index];
				...
				if (0.0 == 0) // Poi Custom Light Color
				{
					...
					poiLight.indirectColor = BetterSH9(float4(lerp(0, poiMesh.normals[1], 0.333), 1));
				}
				...
				if (1.0)
				{
					poiLight.directColor = min(poiLight.directColor, 2.5);
					poiLight.indirectColor = min(poiLight.indirectColor, 2.5);
				}
				if (0.0)
				{
				...
				poiLight.attenuationStrength = 1.0;
				poiLight.directColor = 1.0 ? MaxLuminance(_LightColor0.rgb * poiLight.attenuation, 2.5) : _LightColor0.rgb * poiLight.attenuation;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				poiLight.indirectColor = 0;
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, 0.05);
				poiLight.indirectColor = 1.0 ? MaxLuminance(poiLight.indirectColor, 2.5) : poiLight.indirectColor;
				#endif
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), 0.0);
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), 0.0);
				poiLight.halfDir = normalize(poiLight.direction + poiCam.viewDir);
