Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/520c0c3795796fb439a68661b0844ffc"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void CalculateUTSSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (0.0)
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, 0.4))) * 0.4;
					specMask2 += pow(specArea, exp2(lerp(11, 1, 0.1))) * 0.2;
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(0.4, 5)), 0.0) * 0.4;
					specMask2 += poiEdgeNonLinear(specArea, (1.0 - pow(0.1, 5)), 0.15) * 0.2;
				}
				#if defined(PROP_SET_HIGHCOLORMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (0.0)
						{
							vSpecMask = pow(specArea, exp2(lerp(11, 1, 0.4))) * 0.4 * poiLight.vAttenuation[index];
							vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, 0.1))) * 0.2 * poiLight.vAttenuation[index]);
						}
						else
						{
							vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(0.4, 5)), 0.0) * 0.4 * poiLight.vAttenuation[index];
							vSpecMask = max(vSpecMask, poiEdgeNonLinear(specArea, (1.0 - pow(0.1, 5)), 0.15) * 0.2 * poiLight.vAttenuation[index]);
						}
			...
			float3 lilCalcSpecular(PoiMesh poiMesh, PoiCam poiCam, float3 L, float3 specular, float attenuation, float roughness)
			{
				...
				if (1.0)
				return poiEdgeLinear(pow(nh, 1.0 / roughness), 0.5, 0.0);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.02285505,0.02285505,0.02285505,1).rgb, 0.0);
