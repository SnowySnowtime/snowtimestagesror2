Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/fffc9a264f0cea34e98d4ce8bcac38cd"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 lilCalcSpecular(PoiMesh poiMesh, PoiCam poiCam, float3 L, float3 specular, float attenuation, float roughness)
			{
				...
				if (1.0)
				return poiEdgeLinear(pow(nh, 1.0 / roughness), 0.5, 0.1);
			...
			void lilReflection(inout PoiFragData poiFragData, PoiCam poiCam, PoiLight poiLight, PoiMesh poiMesh, PoiMods poiMods)
			{
				...
				#endif
				{
					...
					#endif
					smoothness = GSAAForSmoothness(smoothness, poiMesh.normals[1], 1.0);
					perceptualRoughness = perceptualRoughness - smoothness * perceptualRoughness;
					float roughness = perceptualRoughness * perceptualRoughness;
					#endif
					float metallic = pow(1.0, 2.2);
					...
					if (1.0)
					{
						...
						if (!SceneHasReflections() || 0.0)
						{
							...
							#endif
							envReflectionColor *= float4(1,1,1,1).rgb * lerp(1.0, poiLight.directColor, 1.0);
						}
						else
						{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.04491682,0.153095,0.01335564,1).rgb, 0.0);
