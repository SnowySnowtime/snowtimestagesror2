Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/a3665acad4265154d91456049f7e958e"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyPositionText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0.13,0,0), 0.0, float4(0.4,0.035,1,1), uv);
				...
				float2 msdfUnit = 4.0 / float4(0.0009765625,0.0009765625,1024,1024).zw;
				...
				globalTextEmission += poiThemeColor(poiMods, float4(1,0,1,1).rgb, 0.0) * opacity * 0.8;
			}
			void ApplyTimeText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0.08,0,0), 0.0, float4(0.4,0.06,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0,0.005828385,1,1).rgb, 0.0), opacity * float4(0,0.005828385,1,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0,0.005828385,1,1).rgb, 0.0) * opacity * 0.7;
			}
			void ApplyFPSText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0,0,0), 0.0, float4(0.4,0.1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0,1,0.9787213,1).rgb, 0.0), opacity * float4(0,1,0.9787213,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0,1,0.9787213,1).rgb, 0.0) * opacity * 2.0;
			}
			void ApplyNumericText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uint wholeDigits = 3.0;
				uint decimalDigits = 2.0;
				...
				float charSign = 420.69 >= 0 ? ASCII_SPACE : ASCII_NEGATIVE;
				NumericArray[arrayIndex] = charSign;						//First character is always the sign
				arrayIndex++;
				if (wholeDigits > 0)
				{
					wholeNumber = uint(glsl_mod(abs(420.69), pow(10, wholeDigits)));
				...
				if (decimalDigits > 0)
				{
					...
					arrayIndex++;
					totalCharacters++;
					decimalNumber = uint(frac(abs(420.69)) * pow(10.00001, decimalDigits));    // Isolate the decimal number
				...
				uv = TransformUV(float4(-0.02,-0.095,0,0), 0.0, float4(0.4,0.1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0.6920712,1,0,1).rgb, 0.0), opacity * float4(0.6920712,1,0,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0.6920712,1,0,1).rgb, 0.0) * opacity * 1.9;
			}
