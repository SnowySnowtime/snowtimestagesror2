Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/c4e75bdfe85bdfb42a5bc0a488037569"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyPathing(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float3 pathEmission;
				#if defined(PROP_PATHINGMAP) || !defined(OPTIMIZER_ENABLED)
				float4 path = _PathingMap.Sample(SmpRepeatPoint, poiUV(poiMesh.uv[0.0], float4(2,2,0,0)) + float4(0,0,0,0).xy * _Time.x);
				#else
				...
				#ifdef POI_AUDIOLINK
				float4 chronoType = float4(0.0, 0.0, 0.0, 0.0);
				float4 chronoBand = float4(0.0, 0.0, 0.0, 0.0);
				float4 chronoSpeed = float4(0.0, 0.0, 0.0, 0.0);
				float3 autoCorrelator[4] = {
					float3(0.0, float4(0.1,0.9,0,1)[0], float4(0.1,0.9,0,1)[1]), float3(0.0, float4(0.1,0.9,0,1)[0], float4(0.1,0.9,0,1)[1]),
					float3(0.0, float4(0.1,0.9,0,1)[0], float4(0.1,0.9,0,1)[1]), float3(0.0, float4(0.1,0.9,0,1)[0], float4(0.1,0.9,0,1)[1])
				};
				float4 history[4] = {
					float4(0.0, 0.0, float4(0,1,0,1)[0], float4(0,1,0,1)[1]), float4(0.0, 0.0, float4(0,1,0,1)[0], float4(0,1,0,1)[1]),
					float4(0.0, 0.0, float4(0,1,0,1)[0], float4(0,1,0,1)[1]), float4(0.0, 0.0, float4(0,1,0,1)[0], float4(0,1,0,1)[1])
				};
				if (poiMods.audioLinkAvailable)
				{
					if (0.0)
					{
						pathAudioLinkPathTimeOffsetBand[0] = 0.0;
						pathAudioLinkPathTimeOffsetBand[1] = 0.0;
						pathAudioLinkPathTimeOffsetBand[2] = 0.0;
						pathAudioLinkPathTimeOffsetBand[3] = 0.0;
					...
					if (0.0)
					{
						pathAudioLinkPathWidthOffsetBand[0] = 0.0;
						pathAudioLinkPathWidthOffsetBand[1] = 0.0;
						pathAudioLinkPathWidthOffsetBand[2] = 0.0;
						pathAudioLinkPathWidthOffsetBand[3] = 0.0;
					...
					if (0.0)
					{
						pathAudioLinkEmission.r += lerp(float4(0,0,0,1).x, float4(0,0,0,1).y, poiMods.audioLink[0.0]);
						pathAudioLinkEmission.g += lerp(float4(0,0,0,1).x, float4(0,0,0,1).y, poiMods.audioLink[0.0]);
						pathAudioLinkEmission.b += lerp(float4(0,0,0,1).x, float4(0,0,0,1).y, poiMods.audioLink[0.0]);
						pathAudioLinkEmission.a += lerp(float4(0,0,0,1).x, float4(0,0,0,1).y, poiMods.audioLink[0.0]);
					}
					if(0.0)
					{
						if (0.0)
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (path[index])
					{
						...
						if (poiMods.audioLinkAvailable)
						{
							...
							if (0.0 && autoCorrelator[index][0] != 0)
							{
				...
				pathEmission = 0;
				pathEmission += pathAlpha[0][1.0] * poiThemeColor(poiMods, PathColor[0].rgb, 0.0) * (float4(4,4,4,4)[0] + pathAudioLinkEmission.r);
				pathEmission += pathAlpha[1][1.0] * poiThemeColor(poiMods, PathColor[1].rgb, 0.0) * (float4(4,4,4,4)[1] + pathAudioLinkEmission.g);
				pathEmission += pathAlpha[2][1.0] * poiThemeColor(poiMods, PathColor[2].rgb, 0.0) * (float4(4,4,4,4)[2] + pathAudioLinkEmission.b);
				pathEmission += pathAlpha[3][1.0] * poiThemeColor(poiMods, PathColor[3].rgb, 0.0) * (float4(4,4,4,4)[3] + pathAudioLinkEmission.a);
				pathEmission *= pathColorMap.rgb * pathColorMap.a;
				float3 colorReplace = 0;
				colorReplace = pathAlpha[0][1.0] * poiThemeColor(poiMods, PathColor[0].rgb, 0.0) * pathColorMap.rgb;
				albedo.rgb = lerp(albedo.rgb, colorReplace + albedo.rgb * 0.00001, pathColorMap.a * PathColor[0].a * pathAlpha[0][1.0]);
				colorReplace = pathAlpha[1][1.0] * poiThemeColor(poiMods, PathColor[1].rgb, 0.0) * pathColorMap.rgb;
				albedo.rgb = lerp(albedo.rgb, colorReplace + albedo.rgb * 0.00001, pathColorMap.a * PathColor[1].a * pathAlpha[1][1.0]);
				colorReplace = pathAlpha[2][1.0] * poiThemeColor(poiMods, PathColor[2].rgb, 0.0) * pathColorMap.rgb;
				albedo.rgb = lerp(albedo.rgb, colorReplace + albedo.rgb * 0.00001, pathColorMap.a * PathColor[2].a * pathAlpha[2][1.0]);
				colorReplace = pathAlpha[3][1.0] * poiThemeColor(poiMods, PathColor[3].rgb, 0.0) * pathColorMap.rgb;
				albedo.rgb = lerp(albedo.rgb, colorReplace + albedo.rgb * 0.00001, pathColorMap.a * PathColor[3].a * pathAlpha[3][1.0]);
				float alpha = max(max(max(pathAlpha[0][1.0], pathAlpha[1][1.0]), pathAlpha[2][1.0]), pathAlpha[3][1.0]);
				poiFragData.alpha *= lerp(1, alpha, 0.0);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0 == 0) // Poi Custom Light Color
				{
				...
				if (1.0 == 1) // More standard approach to light color
				{
