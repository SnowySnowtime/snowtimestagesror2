Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/05876963e63ea1d4ca48f7331a07be97"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					if (8.0 != 0)
					{
						currentFrame = ((_Time.y / (1 / 8.0)) + 0.0) % totalFrames;
						if (0.0)
						{
							currentFrame += 0.0;
						}
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, flipBookPixel.rgb, 2.0), flipBookPixel.a * float4(1,1,1,1).a * 1.0 * flipBookMask * flipbookAlpha);
				float flipbookEmissionStrength = 0.0;
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
