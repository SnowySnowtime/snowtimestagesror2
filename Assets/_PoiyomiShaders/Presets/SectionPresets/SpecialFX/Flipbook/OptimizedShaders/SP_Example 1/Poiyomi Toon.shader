Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/22d90c2db55caeb4cb66beb25e2c8821"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (poiMods.audioLinkAvailable)
				{
					if (0.0)
					{
						currentFrame += AudioLinkGetChronoTime(0.0, 0.0) * 0.0;
					}
				...
				if (poiMods.audioLinkAvailable)
				{
					flipbookEmissionStrength += max(lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[0.0]), 0);
				}
				#endif
