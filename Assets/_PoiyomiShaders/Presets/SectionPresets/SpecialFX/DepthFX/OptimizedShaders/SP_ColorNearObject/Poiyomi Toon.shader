Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/bd29ab85020ea064fa51ead19d707a8e"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDepthFX(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (1.0)
				{
					float colorBlendAlpha = lerp(1.0, 0.0, remapClamped(0.05, 0.1, diff));
					#if defined(PROP_DEPTHTEXTURE) || !defined(OPTIMIZER_ENABLED)
					float2 depthTextureUV = float2(0, 0);
					if (0.0 == 8)
					{
						depthTextureUV = lerp(0, 1, remapClamped(0.05, 0.1, diff));
					}
					else
					{
						depthTextureUV = poiMesh.uv[0.0];
					}
					float3 depthColor = POI2D_SAMPLER_PAN(_DepthTexture, _MainTex, poiUV(depthTextureUV, float4(1,1,0,0)), float4(0,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,0,0,1), 0.0);
					#else
					float3 depthColor = poiThemeColor(poiMods, float4(1,0,0,1), 0.0);
					#endif
					switch(0.0)
					{
						case 0:
				...
				if (0.0)
				{
					poiFragData.alpha *= lerp(poiFragData.alpha, saturate(lerp(0.0, 1.0, remapClamped(0.5, 0.75, diff))), depthMask);
				}
			}
			#endif
