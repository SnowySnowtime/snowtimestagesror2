Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/a78201e1ed7109a409bf47c1bdc344ae"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					float2 st = (poiMesh.uv[0.0] + float4(0,0,0,0).xy * _Time.x) * 300.0;
					...
					half3 glitterColor = poiThemeColor(poiMods, float4(8,8,8,1).rgb, 0.0);
					...
					switch(0.0)
					{
						...
						if (5.0 + glitterSpeedOffset > 0)
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, 30.0, 5.0, glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, 30.0);
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, 0.8), poiCam.viewDir)), 800.0), 0);
						finalGlitter *= glitterAlpha;
						break;
						case 1:
						float randomOffset = random(randoPoint);
						float brightness = (sin((_Time.x * 10 + randomOffset +glitterSpeedOffset) * 5.0) * .5 + .5);
						...
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(specAngle, 800.0), 0);
					...
					glitterMask *= lerp(1, poiLight.rampedLightMap, 0.5);
					glitterMask *= lerp(1, poiLight.directLuminance, 0.5);
					...
					if (0.0 == 1)
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * (3.7 + GlitterbrightnessOffset), finalGlitter * glitterTexture.a * glitterMask);
						poiFragData.emission += finalGlitter * glitterColor * max(0, ((3.7 + GlitterbrightnessOffset) - 1) * glitterTexture.a) * glitterMask;
					}
					else
					{
						poiFragData.emission += finalGlitter * glitterColor * (3.7 + GlitterbrightnessOffset) * glitterTexture.a * glitterMask;
					}
				}
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.03550518,0.03550518,0.03550518,1).rgb, 0.0);
