Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/fd19370832eeea842a5b49505a5c4f25"
{
	Properties
	{
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint(2.0); glitterLayer++)
				{
					...
					switch(0.0)
					{
						...
						if (3.0 + glitterSpeedOffset > 0)
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, 63.0, 3.0, glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, 63.0);
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, 0.2), poiCam.viewDir)), 282.0), 0);
						finalGlitter *= glitterAlpha;
						break;
						case 1:
						float randomOffset = random(randoPoint);
						float brightness = (sin((_Time.x * 10 + randomOffset +glitterSpeedOffset) * 3.0) * .5 + .5);
						...
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(specAngle, 282.0), 0);
					...
					glitterMask *= lerp(1, poiLight.rampedLightMap, 0.8);
					glitterMask *= lerp(1, poiLight.directLuminance, 0.8);
