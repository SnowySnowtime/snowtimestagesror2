Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/e5a42ecf096cbd141b1ea46dd190391c"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					float2 st = (poiMesh.uv[0.0] + float4(0,0,0,0).xy * _Time.x) * 700.0;
					...
					switch(2.0)
					{
						...
						if (0.0 + glitterSpeedOffset > 0)
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, 30.0, 0.0, glitterSpeedOffset);
						}
						else
						{
						...
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, 0.8), poiCam.viewDir)), 200.0), 0);
						finalGlitter *= glitterAlpha;
						break;
						case 1:
						float randomOffset = random(randoPoint);
						float brightness = (sin((_Time.x * 10 + randomOffset +glitterSpeedOffset) * 0.0) * .5 + .5);
						...
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(specAngle, 200.0), 0);
					...
					glitterMask *= lerp(1, poiLight.directLuminance, 0.76);
