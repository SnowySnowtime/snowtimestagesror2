Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/bcab8dd901918cb4eb4acd2d903ce1cb"
{
	Properties
	{
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				switch(1.0)
				{
					default: // Basic (case 1)
					{
						...
						edgeAlpha = remapClamped(da + 0.05, da, noise) * (1 - dissolveAlpha);
						break;
					}
					...
					case 3: // Spherical
					{
						...
						if (0.0)
						{
							...
							edgeAlpha = smoothstep(0.05 + .00001, 0, -normalizedDistance);
						}
						else
						{
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
				...
				if (0.05 || (1.0 == 2 && 0.1 != 0))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0), float4(1,1,1,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * 0.0, dissolveAlpha) + lerp(0, edgeColor.rgb * 1.0, smoothstep(0, 1 - 0.0 * .99999999999, edgeAlpha));
				#endif
			}
			#endif
