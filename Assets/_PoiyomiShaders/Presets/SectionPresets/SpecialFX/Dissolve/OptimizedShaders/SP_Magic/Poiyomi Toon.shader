Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/ad404daa174b1b54e93d3fd35c494644"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[0.0], float4(3,3,0,0)), float4(0,0,0,0));
				#else
				...
				if (0.1 || (1.0 == 2 && 0.1 != 0))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(0.661876,0,1,1).rgb, 0.0), float4(0.661876,0,1,1).a);
					#ifndef POI_SHADOW
