Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/33f8d183175c68244ad2fa34d946b2c9"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[0.0], float4(12,12,0,0)), float4(0,0,0,0));
				#else
				...
				switch(1.0)
				{
					default: // Basic (case 1)
					{
						...
						edgeAlpha = remapClamped(da + 0.1, da, noise) * (1 - dissolveAlpha);
						break;
					}
					...
					case 3: // Spherical
					{
						...
						if (0.0)
						{
							...
							edgeAlpha = smoothstep(0.1 + .00001, 0, -normalizedDistance);
						}
						else
						{
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
				...
				if (0.1 || (1.0 == 2 && 0.1 != 0))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0.2140411,0.401978,1).rgb, 0.0), float4(1,0.2140411,0.401978,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * 0.0, dissolveAlpha) + lerp(0, edgeColor.rgb * 5.0, smoothstep(0, 1 - 0.0 * .99999999999, edgeAlpha));
				#endif
			}
			#endif
