Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/554747b459de1d444a771ab695fa9f9a"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				if (1.0 && poiMods.audioLinkAvailable)
				{
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[0.0], float4(4,4,0,0)), float4(0,0,0,0));
				#else
				...
				switch(1.0)
				{
					...
					{
						da = remap(da, 0, 1, -0.152, 1);
						dissolveAlpha = da;
						dds *= smoothstep(1, 0.99, da) * lerp(1, smoothstep(0, lerp(0.01, 0.1, dds), da), 1.0);
						...
						edgeAlpha = remapClamped(da + 0.152, da, noise) * (1 - dissolveAlpha);
						break;
					}
					...
					case 3: // Spherical
					{
						...
						dissolveAlpha = da;
						dds *= smoothstep(0, 0.2 * dds + 0.01, dissolveAlpha) * lerp(1, smoothstep(1, 1 - 0.2 * dds - 0.01, dissolveAlpha), 1.0);
						...
						if (0.0)
						{
							...
							edgeAlpha = smoothstep(0.152 + .00001, 0, -normalizedDistance);
						}
						else
						{
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
				...
				if (0.152 || (1.0 == 2 && 0.1 != 0))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0.184475,0,1).rgb, 0.0), float4(1,0.184475,0,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * 0.0, dissolveAlpha) + lerp(0, edgeColor.rgb * 11.12, smoothstep(0, 1 - 0.0 * .99999999999, edgeAlpha));
				#endif
			}
			#endif
