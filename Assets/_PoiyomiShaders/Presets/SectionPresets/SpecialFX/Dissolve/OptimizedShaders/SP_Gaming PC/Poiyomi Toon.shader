Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/ac91eb1f4fca8694da6dab0fec5a1bf7"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0,0,1).rgb, 0.0), float4(1,0,0,1).a);
					#ifndef POI_SHADOW
					
					if (1.0)
					{
						edgeColor.rgb = hueShift(edgeColor.rgb, 0.0 + _Time.x * 50.0, 0.0, 1.0);
					}
					#endif
					poiFragData.baseColor = lerp(poiFragData.baseColor, edgeColor.rgb, smoothstep(0, 1 - 0.0 * .99999999999, edgeAlpha));
				}
				poiFragData.emission += lerp(0, dissolveToTexture * 0.0, dissolveAlpha) + lerp(0, edgeColor.rgb * 20.0, smoothstep(0, 1 - 0.0 * .99999999999, edgeAlpha));
				#endif
			}
			#endif
