Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/11cc49b1463fe4240ae384b23baf0f67"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDithering(inout PoiFragData poiFragData, in PoiCam poiCam)
			{
				if (1.0)
				{
					...
					poiFragData.alpha = saturate(poiFragData.alpha - (dither * (1 - poiFragData.alpha) * 1.0));
				}
			}
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (1.0 == 1)
				{
					...
					if (0.0 && 1.0)
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(1,1,1,1).zw)) * 0.25;
						poiFragData.alpha = (poiFragData.alpha - 0.01) / max(fwidth(poiFragData.alpha), 0.0001) + 0.01;
