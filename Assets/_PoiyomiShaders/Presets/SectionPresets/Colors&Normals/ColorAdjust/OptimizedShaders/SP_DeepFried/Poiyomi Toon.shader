Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/ed36fcd884e1f784c9d913e6fc45bdd7"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0 == 1)
				{
					...
					if (1.0)
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, hueShift(poiFragData.baseColor, shift + 3.0 * _Time.x, 0.0, 1.0), hueShiftAlpha.r);
					}
					else
					{
						poiFragData.baseColor = hueShift(poiFragData.baseColor, frac((shift - (1 - hueShiftAlpha.r) + 3.0 * _Time.x)), 0.0, 1.0);
					}
				}
				if (0.0 && 0.0)
				{
