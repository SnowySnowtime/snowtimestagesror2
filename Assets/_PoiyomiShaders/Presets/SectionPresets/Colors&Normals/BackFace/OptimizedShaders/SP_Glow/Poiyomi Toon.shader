Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/149f22d578708e749b912f9c811a7977"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyBackFaceColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				if (!poiMesh.isFrontFace)
				{
					...
					if (1.0)
					{
						backFaceColor.rgb = hueShift(backFaceColor.rgb, frac(0.0 + 3.0 * _Time.x), 0.0, 1.0);
					}
					...
					poiFragData.emission += backFaceColor.rgb * 3.0 * backFaceMask;
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.3612137,0.2772259,0.1575434,1).rgb, 0.0);
