Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/f7aeb9ab10b4b6347a95239ed20720fa"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#if defined(PROP_SKINTHICKNESS) || !defined(OPTIMIZER_ENABLED)
				float subsurface = 1 - POI2D_SAMPLER_PAN(_SkinThicknessMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)).r;
				#else
				float subsurface = 1;
				#endif
				if (0.0)
				...
				if (1.0 != 1)
				{
					subsurface = pow(subsurface, 1.0);
				}
				float skinScattering = saturate(subsurface * 1.0 * 2);
				...
				ambientNormalWorld = normalize(lerp(poiMesh.normals[1], ambientNormalWorld, 0.7));
				...
				poiLight.finalLighting = lerp(poiLight.directColor, min(lerp(poiLight.indirectColor * float4(1,1,1,1), float4(1,1,1,1), 1.0) * poiLight.occlusion + (sss * poiLight.directColor), poiLight.directColor), 1.0 * poiLight.shadowMask) * attenuation;
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.7874123,0.6912138,0.6238179,1).rgb, 0.0);
