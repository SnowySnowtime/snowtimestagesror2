Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon Grab Pass/28d009a006c1c5a43b0c5bc26fb204aa"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			inline float4 Refraction(float indexOfRefraction, float chromaticAberration, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, inout PoiLight poiLight, in PoiFragData poiFragData)
			{
				...
				else
				{
					...
					refractionColor = grabpassBlur(refractedGrab, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * 1.0);
					#else
			...
			void applyGrabEffects(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (1.0 == 1)
				{
					grabpassColor = Refraction(1.544, 0.0, poiMesh, poiCam, poiMods, poiLight, poiFragData).rgb;
				}
				else
				{
					#ifdef CHROMATIC_ABERRATION_LOW
					grabpassColor = grabpassBlur(poiCam.screenUV, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * 1.0);
					#else
				...
				#ifdef POI_PASS_ADD
				poiFragData.baseColor = poiBlend(5.0, float4(poiFragData.baseColor, poiFragData.alpha), 3.0, float4(0, 0, 0, 1), blendFactor);
				#endif
				#ifdef POI_PASS_BASE
				poiFragData.baseColor = poiBlend(5.0, float4(poiFragData.baseColor, poiFragData.alpha), 3.0, float4(grabpassColor / poiLight.finalLighting, 1), blendFactor);
				#endif
			}
			...
			{
				float clearCoatMask = 0.5;
				float smoothness = 0.975;
				float reflectionMask = 0.6;
				float specularMask = 0.6;
				#if defined(PROP_CLEARCOATMAPS) || !defined(OPTIMIZER_ENABLED)
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.5209957,0.3249489,0.3005438,0).rgb, 0.0);
