Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/bd64ff25d55580d4babac484ae4eb86a"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				if ((4.0 /*_TextNumericWholeDigits*/) == 0 && (0.0 /*_TextNumericDecimalDigits*/) == 0)
				{
					return;
				}
				uint wholeNumber = 0;
				uint decimalNumber = 0;
				uint wholeDigits = (4.0 /*_TextNumericWholeDigits*/);
				uint decimalDigits = (0.0 /*_TextNumericDecimalDigits*/);
				...
				float charSign = (0.0 /*_TextNumericValue*/) >= 0 ? ASCII_SPACE : ASCII_NEGATIVE;
				NumericArray[arrayIndex] = charSign;						//First character is always the sign
				arrayIndex++;
				if (wholeDigits > 0)
				{
					wholeNumber = uint(glsl_mod(abs((0.0 /*_TextNumericValue*/)), pow(10, wholeDigits)));
				...
				if (decimalDigits > 0)
				{
					...
					arrayIndex++;
					totalCharacters++;
					decimalNumber = uint(frac(abs((0.0 /*_TextNumericValue*/))) * pow(10.00001, decimalDigits));    // Isolate the decimal number
				...
				uv = TransformUV(float4(0,0,0,0), (0.0 /*_TextNumericRotation*/), float4(1,1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(1,1,1,1).rgb, (0.0 /*_TextNumericColorThemeIndex*/)), opacity * float4(1,1,1,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(1,1,1,1).rgb, (0.0 /*_TextNumericColorThemeIndex*/)) * opacity * (0.0 /*_TextNumericEmissionStrength*/);
			}
